...
class ModflowGwfnpf(mfpackage.MFPackage):
    ...
    alternative_cell_averaging : string
        * alternative_cell_averaging (string) is a text keyword to indicate
        that an alternative method will be used for calculating the conductance
        for horizontal cell connections. The text value for
        alternative\_cell\_averaging can be ``LOGARITHMIC'', ``AMT-
        LMK'', or ``AMT-HMK''. ``AMT-LMK'' signifies that the conductance will
        be calculated using arithmetic-mean thickness and logarithmic-mean
        ...
        alternative\_cell\_averaging, then the harmonic-mean method
    ...
    thickstrt : boolean
        * thickstrt (boolean) indicates that cells having a negative
        icelltype are confined, and their cell thickness for
        conductance calculations will be computed as STRT-BOT rather than TOP-
        BOT.
    cvoptions : [dewatered]
        * dewatered (string) If the DEWATERED keyword is specified, then the
        ...
        and properties of the overlying cell if the head in the underlying cell
        is below its top.
    perched : boolean
        * perched (boolean) keyword to indicate that when a cell is overlying a
        dewatered convertible cell, the head difference used in Darcy's Law is
        equal to the head in the overlying cell minus the bottom elevation of
        the overlying cell. If not specified, then the default is to use the
        head difference between the two cells.
    rewet_record : [wetfct, iwetit, ihdwet]
        ...
        * iwetit (integer) is a keyword and iteration interval for attempting
        to wet cells. Wetting is attempted every iwetit iteration.
        This applies to outer iterations and not inner iterations. If
        iwetit is specified as zero or less, then the value is changed
        to 1.
        * ihdwet (integer) is a keyword and integer flag that determines which
        ...
        If ihdwet is 0, $h = BOT + WETFCT (hm - BOT)$. If
        ihdwet is not 0, $h = BOT + WETFCT (THRESH)$.
    xt3doptions : [rhs]
        * rhs (string) If the RHS keyword is also included, then the
        XT3D additional terms will be added to the right-hand side. If the
        RHS keyword is excluded, then the XT3D terms will be put into
    ...
    k : [double]
        * k (double) is the hydraulic conductivity. For the common case in
        ...
        conductivity and the vertical hydraulic conductivity, then K
        should be assigned as the horizontal hydraulic conductivity,
        K33 should be assigned as the vertical hydraulic conductivity,
        and texttt{K22 and the three rotation angles should not be specified.
    ...
    k22 : [double]
        * k22 (double) is the hydraulic conductivity of the second ellipsoid
        axis; for an unrotated case this is the hydraulic conductivity in the y
        direction. If K22 is not included in the GRIDDATA block, then
        K22 is set equal to $K$. For a regular MODFLOW grid (DIS
        Package is used) in which no rotation angles are specified,
    ...
    k33 : [double]
        * k33 (double) is the hydraulic conductivity of the third ellipsoid
        ...
        conductivity. When anisotropy is applied, K33 corresponds to
        the $K_{33$ tensor component. All included cells ($IDOMAIN >
    ...
    angle1 : [double]
        * angle1 (double) is a rotation angle of the hydraulic conductivity
        ...
        rotations of the hydraulic conductivity ellipsoid. With the $K_{11$,
        $K_{22$, and $K_{33$ axes of the ellipsoid initially aligned with the
        x, y, and z coordinate axes, respectively, angle1 rotates the
        ellipsoid about its $K_{33$ axis (within the x - y plane). A positive
        value represents counter-clockwise rotation when viewed from any point
        on the positive $K_{33$ axis, looking toward the center of the
        ellipsoid. A value of zero indicates that the $K_{11$ axis lies within
        the x - z plane. If angle1 is not specified, default values of
        zero are assigned to angle1, angle2, and
    ...
    angle2 : [double]
        * angle2 (double) is a rotation angle of the hydraulic conductivity
        tensor in degrees. The angle represents the second of three sequential
        ...
        clockwise rotation when viewed from any point on the positive $K_{22$
        axis, looking toward the center of the ellipsoid. A value of zero
        indicates that the $K_{11$ axis lies within the x - y plane. If
        angle2 is not specified, default values of zero are assigned
        to angle2 and angle3; connections that are not user-
        designated as vertical are assumed to be strictly horizontal (that is,
        to have no z component to their orientation); and connection lengths
    ...
    angle3 : [double]
        * angle3 (double) is a rotation angle of the hydraulic conductivity
        ...
        rotations of the hydraulic conductivity ellipsoid. Following the
        rotations by angle1 and angle2 described above,
        angle3 rotates the ellipsoid about its $K_{11$ axis. An array
        can be specified for angle3 only if angle1 and
        ...
        ellipsoid. A value of zero indicates that the $K_{22$ axis lies within
        the x - y plane.
    wetdry : [double]
        * wetdry (double) is a combination of the wetting threshold and a flag
        to indicate which neighboring cells can cause a cell to become wet. If
        wetdry $<$ 0, only a cell below a dry cell can cause the cell
        to become wet. If wetdry $>$ 0, the cell below a dry cell and
        horizontally adjacent cells can cause a cell to become wet. If
        wetdry is 0, the cell cannot be wetted. The absolute value of
        wetdry is the wetting threshold. When the sum of BOT and the
        absolute value of wetdry at a dry cell is equaled or exceeded
        by the head at an adjacent cell, the cell is wetted. wetdry
        must be specified if ``REWET'' is specified in the OPTIONS block. If
        ``REWET'' is not specified in the options block, then wetdry
        can be entered, and memory will be allocated for it, even though it is
        not used.
