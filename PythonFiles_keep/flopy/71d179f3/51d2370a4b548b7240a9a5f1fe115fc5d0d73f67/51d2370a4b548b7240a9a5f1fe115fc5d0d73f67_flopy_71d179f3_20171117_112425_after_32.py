...
class ModflowIms(mfpackage.MFPackage):
    ...
    csv_output_filerecord : [(csv_output : keyword), (fileout : keyword), (csvfile : string)]
        csv_output : keyword to specify that the record corresponds to the comma separated values solver convergence output.
    ...
    outer_hclose : (outer_hclose : double)
        real value defining the head change criterion for convergence of the outer (nonlinear) iterations, in units of length. When the maximum absolute value of the head change at all nodes during an iteration is less than or equal to outer\_hclose, iteration stops. Commonly, outer\_hclose equals 0.01.
    ...
    number_orthogonalizations : (number_orthogonalizations : integer)
        optional integer value defining the interval used to explicitly recalculate the residual of the flow equation using the solver coefficient matrix, the latest head estimates, and the right hand side. For problems that benefit from explicit recalculation of the residual, a number between 4 and 10 is appropriate. By default, number\_orthogonalizations is zero.
