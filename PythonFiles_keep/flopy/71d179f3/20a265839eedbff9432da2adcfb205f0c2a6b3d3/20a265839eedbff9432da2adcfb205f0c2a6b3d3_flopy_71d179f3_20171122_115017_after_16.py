...
class ModflowGwfmaw(mfpackage.MFPackage):
    ...
    flowing_well : (flowing_well : boolean)
        flowing_well : keyword to indicate the well is a flowing well. The flowing\_well option can be used to simulate flowing wells when the simulated well head exceeds the specified drainage elevation.
    ...
    rate_scaling : (rate_scaling : boolean)
        rate_scaling : activate rate scaling. If rate\_scaling is specified, both pump\_elevation and scaling\_length must be specified. rate\_scaling cannot be used with head\_limit.
    pump_elevation : (pump_elevation : double)
        pump_elevation : is the elevation of the multi-aquifer well pump (pump\_elevation). pump\_elevation cannot be less than the bottom elevation (bottom) of the multi-aquifer well. By default, pump\_elevation is set equal to the bottom of the largest GWF node number connected to a MAW well.
    scaling_length : (scaling_length : double)
        scaling_length : height above the pump elevation (scaling\_length) below which the pumping rate is reduced. The default value for scaling\_length is the well radius.
