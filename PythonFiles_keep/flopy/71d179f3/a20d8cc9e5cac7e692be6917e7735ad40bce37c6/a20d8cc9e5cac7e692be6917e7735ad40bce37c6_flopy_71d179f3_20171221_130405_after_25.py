...
class ModflowGwfuzf(mfpackage.MFPackage):
    ...
    save_flows : (save_flows : boolean)
        save_flows : keyword to indicate that UZF flow terms will be written to
    ...
    obs_filerecord : [(obs6_filename : string)]
        obs6_filename : name of input file to define observations for the UZF
    ...
    mover : (mover : boolean)
        mover : keyword to indicate that this instance of the UZF Package can
    ...
    simulate_et : (simulate_et : boolean)
        simulate_et : keyword specifying that ET in the unsaturated (UZF) and
          saturated zones (GWF) will be simulated. ET can be simulated in the
          UZF cell and not the GWF cell by emitting keywords
          LINEAR\_GWET and SQUARE\_GWET.
    linear_gwet : (linear_gwet : boolean)
        linear_gwet : keyword specifying that groundwater ET will be simulated
          using the original ET formulation of MODFLOW-2005.
    square_gwet : (square_gwet : boolean)
        square_gwet : keyword specifying that groundwater ET will be simulated
          by assuming a constant ET rate for groundwater levels between land
          surface (TOP) and land surface minus the ET extinction depth
          (TOP-EXTDP). Groundwater ET is smoothly reduced from the
          PET rate to zero over a nominal interval at TOP-
          EXTDP.
    simulate_gwseep : (simulate_gwseep : boolean)
        simulate_gwseep : keyword specifying that groundwater discharge
          (GWSEEP) to land surface will be simulated. Groundwater
          discharge is nonzero when groundwater head is greater than land
          surface.
    unsat_etwc : (unsat_etwc : boolean)
        unsat_etwc : keyword specifying that ET in the unsaturated zone will be
          simulated as a function of the specified PET rate while the water
          content (THETA) is greater than the ET extinction water
          content (extwc).
    unsat_etae : (unsat_etae : boolean)
        unsat_etae : keyword specifying that ET in the unsaturated zone will be
          simulated simulated using a capillary pressure based formulation.
          Capillary pressure is calculated using the Brooks-Corey retention
          function.
    nuzfcells : (nuzfcells : integer)
        nuzfcells : is the number of UZF cells. More than 1 UZF cell can be
          assigned to a GWF cell; however, only 1 GWF cell can be assigned to a
          single UZF cell. If the MULTILAYER option is used then UZF cells can
          be assigned to GWF cells below (in deeper layers than) the upper most
          active GWF cells.
    ntrailwaves : (ntrailwaves : integer)
        ntrailwaves : is the number of trailing waves. NTRAILWAVES has a
          default value of 7 and can be increased to lower mass balance error
          in the unsaturated zone.
    nwavesets : (nwavesets : integer)
        nwavesets : is the number of UZF cells specified. NWAVSETS has a
          default value of 40 and can be increased if more waves are required
          to resolve variations in water content within the unsaturated zone.
    uzfrecarray : [(iuzno : integer), (cellid : (integer, ...)), (landflag :
      integer), (ivertcon : integer), (surfdep : double), (vks : double), (thtr
      : double), (thts : double), (thti : double), (eps : double), (boundname :
      string)]
        iuzno : integer value that defines the UZF cell number associated with
          the specified PACKAGEDATA data on the line. iuzno must be
          greater than zero and less than or equal to nuzfcells. UZF
        ...
        landflag : integer value set to one for land surface cells indicating
          that boundary conditions can be applied and data can be specified in
          the PERIOD block. A value of 0 specifies a non-land surface cell.
        ivertcon : integer value set to specify underlying UZF cell that
          receives water flowing to bottom of cell. If unsaturated zone flow
          reaches water table before the cell bottom then water is added to GWF
          cell instead of flowing to underlying UZF cell. A value of 0
        ...
        boundname : name of the UZF cell cell. boundname is an ASCII character
          ...
          single quotes.
    uzfperiodrecarray : [(iuzno : integer), (finf : string), (pet : string), (extdp
      : string), (extwc : string), (ha : string), (hroot : string), (rootact :
      string), (aux : double)]
        iuzno : integer value that defines the UZF cell number associated with
          the specified PERIOD data on the line.
        finf : real or character value that defines the applied infiltration
          rate of the UZF cell ($LT^{-1$). If the Options block includes a
        ...
        pet : real or character value that defines the potential
          ...
          zone. If ivertcon is greater than zero then residual
          potential evapotranspiration not satisfied in the UZF cell is applied
          to the underlying UZF and GWF cells. pet is always
          specified, but is only used if SIMULATE\_ET is specified in
          the OPTIONS block. If the Options block includes a
          TIMESERIESFILE entry (see the ``Time-Variable Input''
          section), values can be obtained from a time series by entering the
          time-series name in place of a numeric value.
        extdp : real or character value that defines the evapotranspiration
          extinction depth of the UZF cell. If ivertcon is greater
          than zero and extdp extends below the GWF cell bottom then
          remaining potential evapotranspiration is applied to the underlying
          UZF and GWF cells. extdp is always specified, but is only
          used if SIMULATE\_ET is specified in the OPTIONS block. If
        ...
        extwc : real or character value that defines the evapotranspiration
          extinction water content of the UZF cell. extwc is always
          specified, but is only used if SIMULATE\_ET and
          UNSAT\_ETWC are specified in the OPTIONS block. If the
          Options block includes a TIMESERIESFILE entry (see the
          ``Time-Variable Input'' section), values can be obtained from a time
          series by entering the time-series name in place of a numeric value.
        ha : real or character value that defines the air entry potential
          (head) of the UZF cell. ha is always specified, but is only
          used if SIMULATE\_ET and UNSAT\_ETAE are specified
          in the OPTIONS block. If the Options block includes a
          TIMESERIESFILE entry (see the ``Time-Variable Input''
          section), values can be obtained from a time series by entering the
          time-series name in place of a numeric value.
        hroot : real or character value that defines the root potential (head)
          of the UZF cell. hroot is always specified, but is only used
          if SIMULATE\_ET and UNSAT\_ETAE are specified in
          the OPTIONS block. If the Options block includes a
          TIMESERIESFILE entry (see the ``Time-Variable Input''
          section), values can be obtained from a time series by entering the
          time-series name in place of a numeric value.
        rootact : real or character value that defines the root activity
          function of the UZF cell. rootact is the length of roots in
          a given volume of soil divided by that volume. Values range from 0 to
          about 3 $cm^{-2$, depending on the plant community and its stage of
          development. rootact is always specified, but is only used
          if SIMULATE\_ET and UNSAT\_ETAE are specified in
          the OPTIONS block. If the Options block includes a
          TIMESERIESFILE entry (see the ``Time-Variable Input''
          section), values can be obtained from a time series by entering the
          time-series name in place of a numeric value.
        aux : represents the values of the auxiliary variables for each UZF.
          The values of auxiliary variables must be present for each UZF. The
