...
class Dictionary:
    ...
    def __new__(cls, *args, **kwargs):
        if len(args) < 1 or not isinstance(args[0], DictionaryVersion):
            version = get_default_dictionary_version()
        else:
            version = args[0]

        if version not in cls.__instances__:
            cls.__instances__[version] = cls.__Dictionary(version, **kwargs)

        return cls.__instances__[version]
    ...
    class __Dictionary:
        ...
        def load(self):
            ...
            print("\t[*] Dictionary loaded (version: %s/%d, nb_roots: %d, nb_terms: %d)"%
                  (str(self.version.date), self.version.version_id, len(self.roots), len(self)))
        ...
        def _populate(self, partitions=None, ranks=None, relations=None, parents=None):
            ...
            if partitions is not None:
                ...
                for t, v in partitions.items():
                    self.partitions[t] = {self.terms[tt] for tt in v}
