...
class ModflowSms(Package):
    ...
    def __init__(self, model, hclose=1E-4, hiclose=1E-4, mxiter=100,
                 ...
                 options=None):
        Package.__init__(self, model, extension, 'SMS', unitnumber)
        self.heading = '# SMS for MODFLOW-USG, generated by Flopy.'
        self.url = ' '
        self.hclose = hclose
    ...
    def load(f, model, ext_unit_dict=None):
        ...
        if nonlinmeth != 0 and nopt == 0:
            if model.verbose:
                print('   loading THETA AKAPPA GAMMA AMOMENTUM NUMTRACK BTOL BREDUC RESLIM...')
            line = f.readline()
            ll = line_parse(line)
        ...
        if linmeth == 1 and nopt == 0:
            if model.verbose:
                print ('    loading IACL NORDER LEVEL NORTH IREDSYS RRCTOL IDROPTOL EPSRN')
            line = f.readline()
            ll = line_parse(line)
            iacl = int(ll.pop(0))
            norder = int(ll.pop(0))
