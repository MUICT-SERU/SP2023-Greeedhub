...
class ModflowGwfrcha(mfpackage.MFPackage):
    ...
    readasarrays : boolean
        * readasarrays (boolean) indicates that array-based input will be used
        for the Recharge Package. This keyword must be specified to use array-
        based input.
    fixed_cell : boolean
    ...
    print_flows : boolean
        * print_flows (boolean) keyword to indicate that the list of recharge
        ...
        there is no Output Control option and PRINT\_FLOWS is specified, then
    ...
    obs_filerecord : [obs6_filename]
        * obs6_filename (string) name of input file to define observations for
        ...
        package.
    irch : [integer]
        * irch (integer) irch is the layer number that defines the
        layer in each vertical column where recharge is applied. If
        irch is omitted, recharge by default is applied to cells in
        layer 1. irch can only be used if READASARRAYS is specified in
        the OPTIONS block.
    recharge : [double]
        * recharge (double) is the recharge flux rate ($LT^{-1$). This rate is
    ...
    aux : [double]
        * aux (double) is an array of values for auxiliary variable aux(iaux),
        ...
        part of the auxiliary variables. A separate array can be specified for
        each auxiliary variable. If an array is not specified for an auxiliary
        variable, then a value of zero is assigned. If the value specified here
