...
class ModflowGwfdisv(mfpackage.MFPackage):
    ...
    idomain : [integer]
        * idomain (integer) is an optional array that characterizes the
        existence status of a cell. If the idomain array is not
        specified, then all model cells exist within the solution. If the
        idomain value for a cell is 0, the cell does not exist in the
        ...
        solution. If the idomain value for a cell is 1, the cell
        exists in the simulation. If the idomain value for a cell is
        -1, the cell does not exist in the simulation. Furthermore, the first
        existing cell above will be connected to the first existing cell below.
        This type of cell is referred to as a ``vertical pass through'' cell.
    ...
    cell2drecarray : [icell2d, xc, yc, ncvert, icvert]
        ...
        * icvert (integer) is an array of integer values containing vertex
        ...
        be listed in clockwise order. Cells that are connected must share
        vertices.
