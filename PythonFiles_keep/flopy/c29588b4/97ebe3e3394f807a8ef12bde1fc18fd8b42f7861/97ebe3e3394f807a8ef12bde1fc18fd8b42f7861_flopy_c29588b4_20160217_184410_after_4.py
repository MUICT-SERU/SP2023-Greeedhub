...
class ModflowLak(Package):
    ...
    def get_default_dtype(structured=True):
        if structured:
            dtype = np.dtype([("k", np.int), ("i", np.int), ("j", np.int),
                              ...
                              ("prcplk", np.float32), ("evaplk", np.float32),
                              ("rnf", np.float32), ("wthdrw", np.float32)])
        else:
            dtype = np.dtype([("node", np.int), ("lake", np.int),
                              ("leakance", np.float32), ("prcplk", np.float32), 
                              ("evaplk", np.float32), ("rnf", np.float32), 
                              ("wthdrw", np.float32)])
    ...
    def write_file(self):
        ...
        #                     f.write(fmt8.format(line[idx]))
        #                 f.write('\n')
        #         # dataset 9
        #         if self.ntrib > 0:
        #             for line in sdata:
        #                 for idx in range(3):
        #                     f.write(fmt9.format(line[idx]))
        #                 f.write('\n')
        #         # dataset 10
        #         if self.ndiv > 0:
        #             for line in sdata:
    ...
    def load(f, model, nper=None, ext_unit_dict=None):
        ...
        for iper in range(nper):
            if model.verbose:
                print("   loading " + str(ModflowLak) + " for kper {0:5d}".format(iper + 1))
            # line = f.readline()
            # if line == '':
            #     break
            # t = line.strip().split()
            ...
            #     bnd_output = None
            #     seg_output = None
            #     current, current_lak = ModflowLak.get_empty(itmp, nss, aux_names=aux_names)
            # elif itmp > 0:
            #     if model.verbose:
        ...
        lakpak = ModflowLak(model, options=options)
        return lakpak
