...
class BunqClient(object):
    ...
    def __init__(self,
        ...
        self.headers["User-Agent"] = "bunqclient 2017.7.5"
    ...
    def request(self, request, method="GET", data="", **k):
        order = {self.hierarchy.index(idtype): idtype for idtype in k.keys()}
        elements = [self.base, request]
        elements.extend(["/".join([i, j]) for i, j in sorted(order.items())])
        url = "/".join(elements)
