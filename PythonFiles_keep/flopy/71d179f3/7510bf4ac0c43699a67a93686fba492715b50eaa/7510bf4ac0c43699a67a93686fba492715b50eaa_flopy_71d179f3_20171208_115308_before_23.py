...
class ModflowGwfriv(mfpackage.MFPackage):
    ...
    obs_filerecord : [(obs6_filename : string)]
        obs6_filename : name of input file to define observations for the River package. See the ``Observation utility'' section for instructions for preparing observation input files. Table obstype lists observation type(s) supported by the River package.
    mover : (mover : boolean)
    ...
    periodrecarray : [(cellid : (integer, ...)), (stage : double), (cond : double), (rbot : double), (aux : double), (boundname : string)]
    ...
    """
    auxiliary = mfdatautil.ListTemplateGenerator(('gwf6', 'riv', 'options', 'auxiliary'))
    ts_filerecord = mfdatautil.ListTemplateGenerator(('gwf6', 'riv', 'options', 'ts_filerecord'))
    obs_filerecord = mfdatautil.ListTemplateGenerator(('gwf6', 'riv', 'options', 'obs_filerecord'))
    periodrecarray = mfdatautil.ListTemplateGenerator(('gwf6', 'riv', 'period', 'periodrecarray'))
    package_abbr = "gwfriv"
    ...
    def __init__(self, model, add_to_package_list=True, auxiliary=None, auxmultname=None, boundnames=None,
        ...
        super(ModflowGwfriv, self).__init__(model, "riv", fname, pname, add_to_package_list, parent_file)        
