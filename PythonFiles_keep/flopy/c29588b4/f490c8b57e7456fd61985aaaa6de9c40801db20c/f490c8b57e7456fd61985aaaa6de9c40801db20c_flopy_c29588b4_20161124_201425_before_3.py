...
class ModflowOc(Package):
    ...
    def __init__(self, model, \
                 ...
                 unitnumber=None, filepths=None, **kwargs):
        ...
        for key, value in stress_period_data.items():
            ...
            for t in tlist:
                ...
                if 'save budget' in t.lower():
                    savebud = True
                    if unitnumber[3] == 0 and filepths is None:
                        unitnumber[3] = 53
        ...
        if filepths is not None:
            for idx, pth in enumerate(filepths[1:]):
                if pth is None:
                    continue
    ...
    def load(f, model, nper=None, nstp=None, nlay=None, ext_unit_dict=None):
        ...
        if ext_unit_dict is not None:
            for key, value in ext_unit_dict.items():
                if value.filetype == ModflowOc.ftype():
                    unitnumber[0] = key
        filepths = [None, None, None, None]
        if ihedun > 0:
            unitnumber[1] = ihedun
            #model.add_pop_key_list(ihedun)
            try:
                filepths[1] = ext_unit_dict[ihedun].filename
            except:
                pass
        if iddnun > 0:
            unitnumber[2] = iddnun
            ...
            try:
                filepths[2] = ext_unit_dict[iddnun].filename
            except:
                pass
        if ibouun > 0:
            unitnumber[4] = ibouun
            ...
            try:
                filepths[4] = ext_unit_dict[ibouun].filename
            except:
                pass
            if cboufm is None:
                cboufm = True
        ...
        oc = ModflowOc(model, ihedfm=ihedfm, iddnfm=iddnfm,
                       ...
                       unitnumber=unitnumber, filepths=filepths)
