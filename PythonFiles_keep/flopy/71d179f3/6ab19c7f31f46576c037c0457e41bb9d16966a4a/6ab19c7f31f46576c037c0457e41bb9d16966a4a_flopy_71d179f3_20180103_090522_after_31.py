...
class ModflowTdis(mfpackage.MFPackage):
    ...
    time_units : string
        * time_units (string) is the time units of the simulation. This is a
        ...
        for time\_units may be ``unknown'', ``seconds'', ``minutes'',
        ``hours'', ``days'', or ``years''. The default time unit is
        ``unknown''.
    ...
    tdisrecarray : [perlen, nstp, tsmult]
        ...
        * tsmult (double) is the multiplier for the length of successive time
        ...
        time step, $\Delta t_1$, is related to PERLEN, NSTP, and TSMULT by the
        relation $\Delta t_1= perlen \frac{tsmult - 1{tsmult^{nstp-1$.
