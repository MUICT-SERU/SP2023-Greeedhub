...
class ModflowGwfgwf(mfpackage.MFPackage):
    ...
    auxiliary : [string]
        * auxiliary (string) an array of auxiliary variable names. There is no
        ...
        auxiliary variables will not be used by the GWF-GWF Exchange, but they
        will be available for use by other parts of the program. If an
        auxiliary variable with the name ``ANGLDEGX'' is found, then this
        information will be used as the angle (provided in degrees) between the
        connection face normal and the x axis. Additional information on
    ...
    print_flows : boolean
        * print_flows (boolean) keyword to indicate that the list of exchange
        flow rates will be printed to the listing file for every stress period
        in which ``SAVE BUDGET'' is specified in Output Control.
    save_flows : boolean
    ...
    cell_averaging : string
        * cell_averaging (string) is a keyword and text keyword to indicate the
        method that will be used for calculating the conductance for horizontal
        cell connections. The text value for cell\_averaging can be
    ...
    gnc_filerecord : [gnc6_filename]
        * gnc6_filename (string) is the file name for ghost node correction
        ...
        provided with these keywords. The format for specifying the ghost nodes
        is the same as described for the GNC Package of the GWF Model. This
        includes specifying OPTIONS, DIMENSIONS, and GNCDATA blocks. The order
        of the ghost nodes must follow the same order as the order of the cells
        in the EXCHANGEDATA block. For the GNCDATA, noden and all of the nodej
        values are assumed to be located in model 1, and nodem is assumed to be
        in model 2.
    mvr_filerecord : [mvr6_filename]
        * mvr6_filename (string) is the file name of the water mover input file
        to apply to this exchange. Information for the water mover are provided
        in the file provided with these keywords. The format for specifying the
        ...
        included before pname1 and pname2 in the BEGIN PERIOD
        block. This allows providers and receivers to be located in both models
    ...
    gwfgwfrecarray : [cellidm1, cellidm2, ihc, cl1, cl2, hwva, aux]
        * cellidm1 ((integer, ...)) is the cellid of the cell in model 1 as
        specified in the simulation name file. For a structured grid that uses
        the DIS input file, cellidm1 is the layer, row, and column
        numbers of the cell. For a grid that uses the DISV input file,
        cellidm1 is the layer number and cell2d number for the two
        cells. If the model uses the unstructured discretization (DISU) input
        file, then cellidm1 is the node number for the cell.
        * cellidm2 ((integer, ...)) is the cellid of the cell in model 2 as
        specified in the simulation name file. For a structured grid that uses
        the DIS input file, cellidm2 is the layer, row, and column
        numbers of the cell. For a grid that uses the DISV input file,
        cellidm2 is the layer number and cell2d number for the two
        cells. If the model uses the unstructured discretization (DISU) input
        file, then cellidm2 is the node number for the cell.
        * ihc (integer) is an integer flag indicating the direction between
        node n and all of its m connections. If $ihc=0$ then the connection is
        vertical. If $ihc=1$ then the connection is horizontal. If $ihc=2$ then
        the connection is horizontal for a vertically staggered grid.
        ...
        * hwva (double) is the horizontal width of the flow connection between
        nodem1 and nodem2 if $ihc > 0$, or it is the
        area of the vertical connection between nodem1 and
        nodem2 if $ihc = 0$.
        * aux (double) represents the values of the auxiliary variables for
        each GWFGWF Exchange. The values of auxiliary variables must be present
        for each exchange. The values must be specified in the order of the
