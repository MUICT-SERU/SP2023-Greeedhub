...
class MFObservation:
    ...
    Class is called by the MFSimulation.SimulationDict() class and is not called by the user
    ...
    key: (tuple, stings) user supplied dictionary key to request observation utility data
...
class Observations:
    ...
    get_data(): (np.array) returns array of observation data
        ...
        idx = (int), (slice(start, stop)) interger or slice of data to be returned. corresponds to kstp*kper - 1
        totim = (float) model time value to return data from
        
    list_records(): prints a list of all valid record names contained within the Obs.out file
    ...
    def _reader(self, fi):
        # observation file reader is a standard csv reader that we try to convert each entry to floating point
        with open(fi) as f:
            reader = csv.reader(f)
    ...
    def list_records(self):
        # requester option to list all records (observation names) within an observation file
    ...
    def get_data(self, key=None, idx=None, totim=None):
        ...
        # check if user supplied observation key, default is to return all observations
        if key is None:
            header = data[0]
            if idx is not None:
                data = data[idx, :]
    ...
    def get_dataframe(self, keys=None, idx=None, totim=None, start_datetime=None, timeunit='D'):
        '''
        Creates a pandas dataframe object from the observation data, useful backend if the user does not like the
        x-array format!

        Parameters
        ----------
        keys: (string) sting of dictionary/observation keys seperated by comma. (optional)
        ...
        # check to see if user supplied keys, if not get all observations, adjust for time if necessary.
        if keys is None:
            if idx is not None or totim is not None:
                ...
                # use dictionary comprehension to create a set of pandas series that can be added to a pd.DataFrame
                d = {key: pd.Series(data[key][idx], index=[time[idx]]) for key in data if key != 'time'}
            else:
                d = {key: pd.Series(data[key], index=time) for key in data if key != 'time'}
                
        else:
            keys = self._key_list(keys)
            for key in keys:
                if key not in data:
                    raise KeyError('Supplied data key: {} is not valid'.format(key))
                else:
                    pass
            ...
            if idx is not None or totim is not None:
                ...
                d = {key: pd.Series(data[key][idx], index=[time[idx]]) for key in data if key != 'time' and key in keys}
            else:
                d = {key: pd.Series(data[key], index=time) for key in data if key != 'time' and key in keys}
    ...
    def _key_list(self, keys):
        # check if user supplied keys is single or multiple, string or list. Return a list of keys.
        key_type = type(keys)
        if key_type is str:
            keys = keys.split(',')
            ...
            err = 'Invalid key type: supply a strg of keys seperated by , or a list of keys'
            raise TypeError(err)
        return keys
    ...
    def _get_datetime(self, times, start_dt, unit):
        ...
        # check user supplied format of datetime, is it dd/mm/yyyy or dd/mm/yyyy hh:mm:ss?
        if ':' in start_dt:
        ...
        try:    
            ...
            err = 'please supply start_datetime in the format "dd/mm/yyyy hh:mm:ss" or "dd/mm/yyyy"'
            raise AssertionError(err)

        # create list of datetimes
        t0 = dt.datetime(year, month, day, hour, minute, second)
        if unit is 'Y':
            dtlist = [dt.datetime(int(year + time), month, day, hour, minute, second) for time in times]
        elif unit is 'D':
    ...
    def get_obs_data(self, key=None, idx=None, totim=None):
        ...
        Parameters
        ----------
        key: (string) dictionary key for a specific observation contained within the observation file (optional)
        idx: (int) time index (optional)
        totim: (float) simulation time (optional)
...
class MFObservationRequester:
    '''
    Wrapper class for MFObservation.Observations. Class checks which observation data is available, and creates a
    dictionary key to access the set of observation data from the SimulationDict()
    '''
    def __init__(self, mfdict, path, key, **kwargs):
        self.mfdict = mfdict
        self.path = path
        self.obs_dataDict = {}
        # check that observation files exist, create a key and path to them and set to self.obs_dataDict
        self._check_for_observations()

        # check if user supplied dictionary key is valid, or if it is a dummy key for a key request.
    ...
    def _check_for_observations(self):
        '''
        Checks all entries of mfdict for the string 'observation-input-filenames', finds path to file, creates
        ...
        '''
        possible_observations = [k for k in self.mfdict if 'observation-input-filename' in k and 'FORMAT' not in k]
        partial_key = []
        ...
        for line in partial_key:
            ...
            if check > 1:
                ...
                for i in range(len(multi_observations)):
                    ...
                    self._get_obsfile_names(multi_observations[i], obs8_file, 'CONTINUOUS')
                ...
                raise KeyError('There are no observation files associated with this project')

    def _get_obsfile_names(self, partial_key, OBS8, obstype):
        '''
        Creates a data dictionary key for user to request data. This key holds the path to the observation file
        ...
        '''
        try:
            obstypes = self.mfdict[(partial_key[0], partial_key[1], OBS8, obstype, 'obstype')]
            obspackage = self._get_package_type(obstypes)
            obs_fname = self.mfdict[(partial_key[0], partial_key[1], OBS8, obstype, 'obs_output_file_name')]
            self.obs_dataDict[(partial_key[0], obspackage, obstype, 'Observations')] = obs_fname
        except KeyError:
            pass

    def _get_package_type(self, obstypes):
        # check the observation name in the OBS8 dictionary to get the package type
        valid_packages = ('CHD', 'DRN', 'GHB', 'GWF', 'LAK', 'MAW', 'RIV', 'SFR', 'UZF', 'WEL')
        ...
        if package in valid_packages:
            ...
            raise KeyError('{} is not a valid observation type'.format(package))

    @staticmethod
    def getkeys(mfdict, path):
        # staticmethod to return a valid set of mfdict keys to the user to access this data
