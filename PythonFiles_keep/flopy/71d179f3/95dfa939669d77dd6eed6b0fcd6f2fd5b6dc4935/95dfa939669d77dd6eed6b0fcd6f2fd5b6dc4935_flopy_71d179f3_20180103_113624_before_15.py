...
class ModflowGwflak(mfpackage.MFPackage):
    ...
    print_stage : boolean
        * print_stage (boolean) keyword to indicate that the list of lake
        ...
        Output Control option and PRINT\_STAGE is specified, then stages are
    ...
    obs_filerecord : [obs6_filename]
        * obs6_filename (string) name of input file to define observations for
        ...
        obstype lists observation type(s) supported by the LAK
        package.
    mover : boolean
    ...
    surfdep : double
        * surfdep (double) real value that defines the surface depression depth
        for VERTICAL lake-GWF connections. If specified,
        surfdep must be greater than or equal to zero. If
        SURFDEP is not specified, a default value of zero is used for
        all vertical lake-GWF connections.
    time_conversion : double
        * time_conversion (double) value that is used in converting outlet flow
        ...
        consistent time units. time\_conversion should be set to 1.0,
        ...
        specified if no lake outlets are specified or time\_units are
        seconds.
    length_conversion : double
        * length_conversion (double) real value that is used in converting
        ...
        acceleration to consistent length units. length\_conversion
        should be set to 3.28081, 1.0, and 100.0 when using length units
        (length\_units) of feet, meters, or centimeters in the
        simulation, respectively. length\_conversion does not need to
        be specified if no lake outlets are specified or length\_units
        are meters.
    nlakes : integer
    ...
    noutlets : integer
        * noutlets (integer) value specifying the number of outlets that will
        be simulated for all stress periods. If NOUTLETS is not
    ...
    ntables : integer
        * ntables (integer) value specifying the number of lakes tables that
        ...
        area. If NTABLES is not specified, a default value of zero is
        used.
    lakrecarray_package : [lakeno, strt, nlakeconn, aux, boundname]
        * lakeno (integer) integer value that defines the lake number
        ...
        lakeno must be greater than zero and less than or equal to
        nlakes. Lake information must be specified for every lake or
        ...
        * nlakeconn (integer) integer value that defines the number of
        GWF nodes connected to this (lakeno) lake. There can
        only be one vertical lake connection to each GWF node.
        ...
        * aux (double) represents the values of the auxiliary variables for
        ...
        variables specified in the OPTIONS block. If the package supports time
        series and the Options block includes a TIMESERIESFILE entry (see the
        ...
        * boundname (string) name of the lake cell. boundname is an ASCII
        ...
        boundname contains spaces in it, then the entire name must be enclosed
        within single quotes.
    lakrecarray : [lakeno, iconn, cellid, claktype, bedleak, belev, telev, connlen,
      connwidth]
        * lakeno (integer) integer value that defines the lake number
        associated with the specified CONNECTIONDATA data on the line.
        lakeno must be greater than zero and less than or equal to
        nlakes. Lake connection information must be specified for
        every lake connection to the GWF model (nlakeconn) or the
        ...
        * iconn (integer) integer value that defines the GWF
        ...
        * claktype (string) character string that defines the lake-GWF
        ...
        to indicate the lake-GWF connection is vertical and connection
        conductance calculations use the hydraulic conductivity corresponding
        to the $K_{33$ tensor component defined for cellid in the NPF
        package. HORIZONTAL--character keyword to indicate the
        lake-GWF connection is horizontal and connection conductance
        calculations use the hydraulic conductivity corresponding to the
        $K_{11$ tensor component defined for cellid in the NPF
        package. EMBEDDEDH--character keyword to indicate the
        lake-GWF connection is embedded in a single cell and
        connection conductance calculations use the hydraulic conductivity
        corresponding to the $K_{11$ tensor component defined for
        cellid in the NPF package. EMBEDDEDV--character
        ...
        conductivity corresponding to the $K_{33$ tensor component defined for
        cellid in the NPF package. Embedded lakes can only be
        connected to a single cell (nlakconn = 1) and there must be a
        lake table associated with each embedded lake.
        * bedleak (double) character string or real value that defines the bed
        leakance for the lake-GWF connection. bedleak must be
        ...
        * belev (double) real value that defines the bottom elevation for a
        ...
        HORIZONTAL and belev is not equal to telev,
        belev must be greater than or equal to the bottom of the
        GWF cell cellid. If belev is equal to
        telev, belev is reset to the bottom of the
        GWF cell cellid.
        * telev (double) real value that defines the top elevation for a
        HORIZONTAL lake-GWF connection. Any value can be
        specified if claktype is VERTICAL,
        EMBEDDEDH, or EMBEDDEDV. If claktype is
        HORIZONTAL and telev is not equal to belev,
        telev must be less than or equal to the top of the
        GWF cell cellid. If telev is equal to
        belev, telev is reset to the top of the GWF
        cell cellid.
        * connlen (double) real value that defines the distance between the
        connected GWF cellid node and the lake for a
        ...
        * connwidth (double) real value that defines the connection face width
        for a HORIZONTAL lake-GWF connection.
        connwidth must be greater than zero for a HORIZONTAL
        lake-GWF connection. Any value can be specified if
        claktype is VERTICAL, EMBEDDEDH, or
        EMBEDDEDV.
    lake_tablesrecarray : [lakeno, tab6_filename]
        * lakeno (integer) integer value that defines the lake number
        associated with the specified TABLES data on the line. lakeno
        ...
        * tab6_filename (string) character string that defines the path and
        ...
        connection. The ctabname file includes the number of entries
        in the file and the relation between stage, surface area, and volume
        for each entry in the file. Lake table files for EMBEDDEDH and
        EMBEDDEDV lake-GWF connections also include
        lake-GWF exchange area data for each entry in the file. Input
        instructions for the ctabname file is included at the LAK
        package lake table file input instructions section.
    outletsrecarray : [outletno, lakein, lakeout, couttype, invert, width, rough,
      slope]
        * outletno (integer) integer value that defines the outlet number
        associated with the specified OUTLETS data on the line.
        outletno must be greater than zero and less than or equal to
        noutlets. Outlet information must be specified for every
        ...
        * lakein (integer) integer value that defines the lake number that
        outlet is connected to. lakein must be greater than zero and
        less than or equal to nlakes.
        * lakeout (integer) integer value that defines the lake number that
        outlet discharge from lake outlet outletno is routed to.
        lakeout must be greater than or equal to zero and less than or
        equal to nlakes. If lakeout is zero, outlet discharge
        from lake outlet outletno is discharged to an external
        boundary.
        * couttype (string) character string that defines the outlet type for
        the outlet outletno. Possible couttype strings
        include: SPECIFIED--character keyword to indicate the outlet
        is defined as a specified flow. MANNING--character keyword to
        ...
        * invert (double) real value that defines the invert elevation for the
        lake outlet. Any value can be specified if couttype is
        ...
        * width (double) real value that defines the width of the lake outlet.
        Any value can be specified if couttype is SPECIFIED.
    ...
    lakeperiodrecarray : [lakeno, laksetting]
        * lakeno (integer) integer value that defines the lake number
        associated with the specified PERIOD data on the line. lakeno
        must be greater than zero and less than or equal to nlakes.
    ...
    rate : string
        * rate (string) real or character value that defines the extraction
        ...
        negative value indicates outflow from the lake. rate only
        applies to active (IBOUND$>0$) lakes. A specified
        rate is only applied if couttype for the
        outletno is SPECIFIED. If the Options block includes
    ...
    outletperiodrecarray : [outletno, outletsetting]
        ...
        * outletsetting (keystring) line of information that is parsed into a
        keyword and values. Keyword values that can be used to start the
        outletsetting string include: RATE, INVERT,
        WIDTH, SLOPE, and ROUGH.
