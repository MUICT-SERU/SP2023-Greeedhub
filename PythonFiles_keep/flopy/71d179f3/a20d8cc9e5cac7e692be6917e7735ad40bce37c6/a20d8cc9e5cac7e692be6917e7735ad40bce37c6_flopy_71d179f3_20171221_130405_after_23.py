...
class ModflowGwfsfr(mfpackage.MFPackage):
    ...
    boundnames : (boundnames : boolean)
        boundnames : keyword to indicate that boundary names may be provided
          with the list of stream reach cells.
    print_input : (print_input : boolean)
        print_input : keyword to indicate that the list of stream reach
          information will be written to the listing file immediately after it
          is read.
    print_stage : (print_stage : boolean)
        print_stage : keyword to indicate that the list of stream reach stages
          will be printed to the listing file for every stress period in which
          ``HEAD PRINT'' is specified in Output Control. If there is no Output
          Control option and PRINT\_STAGE is specified, then stages are printed
          for the last time step of each stress period.
    print_flows : (print_flows : boolean)
        print_flows : keyword to indicate that the list of stream reach flow
    ...
    save_flows : (save_flows : boolean)
        save_flows : keyword to indicate that stream reach flow terms will be
    ...
    obs_filerecord : [(obs6_filename : string)]
        obs6_filename : name of input file to define observations for the SFR
    ...
    mover : (mover : boolean)
        mover : keyword to indicate that this instance of the SFR Package can
    ...
    maximum_iterations : (maximum_iterations : double)
        maximum_iterations : value that defines an maximum number of Streamflow
          Routing Newton-Raphson iterations allowed for a reach. By default,
          maxsfrit is equal to 100.
    maximum_depth_change : (maximum_depth_change : double)
        maximum_depth_change : value that defines the depth closure tolerance.
          By default, dmaxchg is equal to $1 \times 10^{-5$.
    unit_conversion : (unit_conversion : double)
        unit_conversion : value (or conversion factor) that is used in
    ...
    nreaches : (nreaches : integer)
        nreaches : integer value specifying the number of stream reaches. There
          must be nreaches entries in the PACKAGEDATA block.
    sfrrecarray : [(rno : integer), (cellid : (integer, ...)), (rlen : double),
      (rwid : double), (rgrd : double), (rtp : double), (rbth : double), (rhk :
      double), (man : string), (ncon : integer), (ustrf : double), (ndv :
      integer), (aux : double), (boundname : string)]
        rno : integer value that defines the reach number associated with the
          specified PACKAGEDATA data on the line. rno must be greater
          than zero and less than or equal to nreaches. Reach
        ...
        cellid : The keyword `none' must be specified for reaches that
          are not connected to an underlying GWF cell. The keyword
          `none' is used for reaches that are in cells that have
          IDOMAIN values less than one or are in areas not covered by
          the GWF model grid. Reach-aquifer flow is not calculated if the
          keyword `none' is specified.
        rlen : real value that defines the reach length. rlen must be
          greater than zero.
        rwid : real value that defines the reach width. rwid must be
          greater than zero.
        rgrd : real value that defines the stream gradient (slope) across the
          reach. rgrd must be greater than zero.
        ...
        rbth : real value that defines the thickness of the reach streambed.
          rbth can be any value if cellid is `none'.
          Otherwise, rbth must be greater than zero.
        rhk : real value that defines the hydraulic conductivity of the reach
          streambed. rhk can be any positive value if cellid
          is `none'. Otherwise, rhk must be greater than
          zero.
        man : real or character value that defines the Manning's roughness
          coefficient for the reach. man must be greater than zero. If
        ...
        ncon : integer value that defines the number of reaches connected to
          the reach.
        ustrf : real value that defines the fraction of upstream flow from each
          upstream reach that is applied as upstream inflow to the reach. The
          sum of all ustrf values for all reaches connected to the
          same upstream reach must be equal to one and ustrf must be
          greater than or equal to zero.
        ndv : integer value that defines the number of downstream diversions
          for the reach.
        aux : represents the values of the auxiliary variables for each stream
          reach. The values of auxiliary variables must be present for each
          stream reach. The values must be specified in the order of the
          ...
          of a numeric value.
        boundname : name of the stream reach cell. boundname is an ASCII
    ...
    reach_connectivityrecarray : [(rno : integer), (ic : integer)]
        rno : integer value that defines the reach number associated with the
          specified CONNECTIONDATA data on the line. rno must be
          ...
          than once.
        ic : integer value that defines the reach number of the reach connected
          to the current reach and whether it is connected to the upstream or
          downstream end of the reach. Negative ic numbers indicate
          connected reaches are connected to the downstream end of the current
          reach. Positive ic numbers indicate connected reaches are
          connected to the upstream end of the current reach. The absolute
          value of ic must be greater than zero and less than or equal
          to NREACHES.
    reach_diversionsrecarray : [(rno : integer), (idv : integer), (iconr :
      integer), (cprior : string)]
        rno : integer value that defines the reach number associated with the
          specified DIVERSIONS data on the line. rno must be greater
          than zero and less than or equal to NREACHES. Reach
          diversion information must be specified for every reach with a
          ndv value greater than 0 or the program will terminate with
        ...
        idv : integer value that defines the downstream diversion number for
          the diversion for reach rno. idv must be greater
          than zero and less than or equal to ndv for reach
          rno.
        iconr : integer value that defines the downstream reach that will
          receive the diverted water. idv must be greater than zero
          and less than or equal to NREACHES. Furthermore, reach
          iconr must be a downstream connection for reach
          rno.
        cprior : character string value that defines the the prioritization
          system for the diversion, such as when insufficient water is
          available to meet all diversion stipulations, and is used in
          conjunction with the value of flow value specified in the
          STRESS\_PERIOD\_DATA section. Available diversion options
          include: (1) cprior = `FRACTION', then the amount of the
          diversion is computed as a fraction of the streamflow leaving reach
          rno ($Q_{DS$); in this case, 0.0 $\le$ divflow
          $\le$ 1.0. (2) cprior = `EXCESS', a diversion is made only
          if $Q_{DS$ for reach rno exceeds the value of
          divflow. If this occurs, then the quantity of water diverted
          is the excess flow ($Q_{DS -$ divflow) and $Q_{DS$ from
          reach rno is set equal to divflow. This represents
          a flood-control type of diversion, as described by Danskin and Hanson
          (2002). (3) cprior = `THRESHOLD', then if $Q_{DS$ in reach
          rno is less than the specified diversion flow
          (divflow), no water is diverted from reach rno. If
          $Q_{DS$ in reach rno is greater than or equal to
          (divflow), (divflow) is diverted and $Q_{DS$ is
          set to the remainder ($Q_{DS -$ divflow)). This approach
          assumes that once flow in the stream is sufficiently low, diversions
          from the stream cease, and is the `priority' algorithm that
          originally was programmed into the STR1 Package (Prudic, 1989). (4)
          cprior = `UPTO' -- if $Q_{DS$ in reach rno is
          greater than or equal to the specified diversion flow
          (divflow), $Q_{DS$ is reduced by divflow. If
          $Q_{DS$ in reach rno is less than (divflow),
          divflow is set to $Q_{DS$ and there will be no flow
          available for reaches connected to downstream end of reach
          rno.
    reachperiodrecarray : [(rno : integer), (sfrsetting : keystring)]
        rno : integer value that defines the reach number associated with the
          specified PERIOD data on the line. rno must be greater than
          zero and less than or equal to NREACHES.
        sfrsetting : line of information that is parsed into a keyword and
          values. Keyword values that can be used to start the
          sfrsetting string include: STATUS,
          MANNING, STAGE, INFLOW, RAINFALL,
          EVAPORATION, RUNOFF, DIVERSION,
          UPSTREAM\_FRACTION, and AUXILIARY.
    diversion : (diversion : boolean)
        diversion : keyword to indicate diversion record.
    idv : (idv : integer)
        idv : diversion number.
    divrate : (divrate : double)
        divrate : real or character value that defines the volumetric diversion
          (divflow) rate for the streamflow routing reach. If the
