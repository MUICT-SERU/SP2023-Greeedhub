...
class IEMLObjects(TreeStructure, metaclass=IEMLType):
    ...
    def path(self, path):
        ...
        p = path[0]
        return [k for e in self._resolve_coordinates(p) for k in e.path(IEMLPath(path[1:]))]
    ...
    def paths(self):
        if not self._paths:
            self._paths = [p * c_p for p, c in self._paths_children() for c_p in c.paths] \
                if self.children else [IEMLPath([self])]
