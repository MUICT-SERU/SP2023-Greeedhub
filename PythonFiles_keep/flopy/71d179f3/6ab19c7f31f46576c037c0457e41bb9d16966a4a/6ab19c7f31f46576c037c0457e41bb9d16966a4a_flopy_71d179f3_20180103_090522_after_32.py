...
class ModflowUtlobs(mfpackage.MFPackage):
    ...
    precision : double
        * precision (double) Keyword and precision specifier for output of
        ...
        observations, time values) are written as single- or double-precision.
    digits : integer
        * digits (integer) Keyword and an integer digits specifier used for
        ...
        type DATA in the Name File, the digits specifier controls the number of
    ...
    print_input : boolean
        * print_input (boolean) keyword to indicate that the list of
        observation information will be written to the listing file immediately
        after it is read.
    continuousrecarray : [obsname, obstype, id, id2]
        * obsname (string) string of 1 to 40 nonblank characters used to
        ...
        identification and post-processing of observations in the output files
        are facilitated if each observation is given a unique name.
        ...
        * id (string) Text identifying cell where observation is located. For
        ...
        corresponding package input file, ID can be a boundary name. Otherwise
        ID is a cellid. If the model discretization is type DIS, cellid is
        three integers (layer, row, column). If the discretization is DISV,
        cellid is two integers (layer, cell number). If the discretization is
        DISU, cellid is one integer (node number).
        * id2 (string) Text identifying cell adjacent to cell identified by ID.
        The form of ID2 is as described for ID. ID2 is used for intercell-flow
