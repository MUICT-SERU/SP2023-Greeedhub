...
class MFComment(object):
    ...
    is_empty(include_whitespace=True)
        checks to see if comment is just an empty string ''.  if include_whitespace
        is set to false a string with only whitespace is considered empty
    is_comment(text, include_empty_line=False) : boolean
        returns true if text is a comment.  an empty line is considered a comment
        if include_empty_line is true.
    ...
    def __init__(self, comment, path, sim_data, line_number=0):
        assert(isinstance(comment, str) or isinstance(comment, list) or comment is None)
        self.text = comment
        self.path = path
        ...
        if base_list and self.sim_data.comments_on:
            for item in base_list:
                if not isinstance(item, str) and isinstance(item, list):
                    file_entry = '{}{}'.format(file_entry, self._recursive_get(item))
                else:
    ...
    def write(self, fd, eoln_suffix=True):
        if self.text and self.sim_data.comments_on:
            # extract text embedded in deep list
            #base_list = self.text
            #while len(base_list) == 1 and not isinstance(base_list, str) and isinstance(base_list, list):
    ...
    def is_comment(text, include_empty_line=False):
        ...
        if include_empty_line and not text_clean:
            return True
        if text_clean and (text_clean[0] == '#' or text_clean[0] == '!' or text_clean[0] == '//'):
            return True
        return False
...
class LayerStorage(object):
    def __init__(self, data_storage, lay_num, data_storage_type=DataStorageType.internal_array,
...
class DataStorage(object):
    ...
    data_storage_type : enum
        how the data will be stored (internally, as a constant, as an external file)
    ...
    update_item(data, key_index)
        updates the data in a recarray at index "key_index" with data "data".  data is a list containing all
        data for a single record in the recarray.  .  data structure type must be recarray
    append_data(data)
        appends data "data" to the end of a recarray.  data structure type must be recarray
    set_data(data, layer=None, multiplier=[1.0]
        sets the data being stored to "data" for layer "layer", replacing all data for that layer.  a multiplier
        can be specified.
    ...
    store_external(file_path, layer=None, multiplier=[1.0], print_format=None, data=None, do_not_verify=False)
        store data "data" at layer "layer" externally in file "file_path"
    ...
    external_to_internal(layer_num=None, store_internal=False) : ndarray/recarray
        loads existing external data for layer "layer_num" and returns it.  if store_internal is True it
        also storages the data internally, changing the storage type for "layer_num" layer to internal.
    ...
    read_data_from_file(layer, fd=None, multiplier=None) : (ndarray, int)
        reads in data from a given file "fd" as data from layer "layer".  returns data as an ndarray along
        with the size of the data
    to_string(val, type, is_cellid=False, possible_cellid=False)
        converts data "val" of type "type" to a string.  is_cellid is True if the data type is known to be
        a cellid and is treated as such.  when possible_cellid is True the data is checked to see if it
        matches the shape/dimensions of a cellid before using it as one.
    resolve_data_size(index) : int
        resolves the size of a given data element in a recarray based on the names in the existing rec_array.
        assumes repeating data element names follow the format <data_element_name>_X.  returns the number of
    ...
    """
    def __init__(self, sim_data, data_dimensions, data_storage_type=DataStorageType.internal_array,
                 data_structure_type=DataStructureType.ndarray, num_layers=1, layered=False):
        self.data_dimensions = data_dimensions
        self._simulation_data = sim_data
        self._data_type_overrides = {}
        self.layer_storage = [LayerStorage(self, x, data_storage_type) for x in range(num_layers)]
        self.num_layers = num_layers
        self.data_structure_type = data_structure_type
        self.in_model = self.data_dimensions is not None and \
                         len(self.data_dimensions.package_dim.package_path) > 1 and \
                         self.data_dimensions.package_dim.model_dim[0].model_name.lower() == \
                         self.data_dimensions.package_dim.package_path[0]
    ...
    def make_layered(self):
        if not self.layered:
            ...
            self.layer_storage = [LayerStorage(self, x, previous_storage.data_storage_type) for x in range(data_dim[0])]
            if previous_storage.data_storage_type == DataStorageType.internal_constant:
                for storage in self.layer_storage:
                    storage._data_const_value = previous_storage._data_const_value
            elif previous_storage.data_storage_type == DataStorageType.internal_array:
        ...
        for index, storage in enumerate(self.layer_storage):
            if storage.data_storage_type == DataStorageType.internal_array:
                if storage._internal_data is not None:
                    ...
                    if formal:
                        data_str = '{}Layer_{}{{{}}}\n({})\n'.format(data_str, index + 1, header, repr(storage))
                    else:
                        data_str = '{}{{{}}}\n({})\n'.format(data_str, header, str(storage))
            elif storage.data_storage_type == DataStorageType.internal_constant:
                if storage._data_const_value is not None:
                    data_str = '{}{{{}}}\n'.format(data_str, self._get_layer_header_str(index))

        return data_str

    def _get_layer_header_str(self, layer):
        header_list = []
        if self.layer_storage[layer].data_storage_type == DataStorageType.external_file:
            header_list.append('open/close {}'.format(self.layer_storage[layer].fname))
        elif self.layer_storage[layer].data_storage_type == DataStorageType.internal_constant:
            header_list.append('constant {}'.format(self.layer_storage[layer]))
        else:
            header_list.append('internal')
        if self.layer_storage[layer].factor != 1.0 and self.layer_storage[layer].factor != 1:
            header_list.append('factor {}'.format(self.layer_storage[layer].factor))
        if self.layer_storage[layer].iprn is not None:
            header_list.append('iprn {}'.format(self.layer_storage[layer].iprn))
    ...
    def add_layer(self):
        self.layer_storage.append(LayerStorage(self, len(self.layer_storage), self.layer_storage[0].data_storage_type))
        self.num_layers += 1
    ...
    def get_const_val(self, layer=None):
        if layer is None:
            ...
            assert(self.layer_storage[0].data_storage_type == DataStorageType.internal_constant)
            return self.layer_storage[0]._data_const_value
        else:
            assert(len(self.layer_storage) > layer)
            assert(self.layer_storage[layer].data_storage_type == DataStorageType.internal_constant)
    ...
    def _access_data(self, layer, return_data=False, apply_mult=True):
        ...
        if self.layer_storage[layer_check].data_storage_type == DataStorageType.external_file:
            if return_data:
            ...
            if (self.layer_storage[layer_check]._internal_data is None and
              self.layer_storage[layer_check].data_storage_type == DataStorageType.internal_array) or \
              (self.layer_storage[layer_check]._data_const_value is None and
              self.layer_storage[layer_check].data_storage_type == DataStorageType.internal_constant):
                return None
            ...
            if layer is None:
                if self.data_structure_type == DataStructureType.ndarray or \
                  self.data_structure_type == DataStructureType.scalar:
                    if return_data:
                        ...
                        if data is None:
                            if self.layer_storage[0].data_storage_type == DataStorageType.internal_constant:
                                return self.layer_storage[0].get_data()[0]
                        else:
                            return data
                    else:
                        if self.data_structure_type == DataStructureType.scalar:
                            return self.layer_storage[0]._internal_data is not None
                        return (self.layer_storage[layer_check]._data_const_value is not None and
                                self.layer_storage[layer_check].data_storage_type == DataStorageType.internal_constant) or (
                                self.layer_storage[layer_check]._internal_data is not None and
                                self.layer_storage[layer_check].data_storage_type == DataStorageType.internal_array)
                else:
                    if self.layer_storage[layer_check].data_storage_type == DataStorageType.internal_constant:
                        if return_data:
                            # recarray stored as a constant.  currently only support grid-based constant
                            # recarrays.  build a recarray of all cells
                            data_list = []
                            for cellid in self.data_dimensions.get_model_grid().get_all_model_cells():
                                data_line = (cellid,) + (self.layer_storage[0]._data_const_value,)
                                if len(self.data_dimensions.structure.data_item_structures) > 2:
                                    # append None any expected optional data
                                    for data_item_struct in self.data_dimensions.structure.data_item_structures[2:]:
                                        if (data_item_struct.name != 'boundname' or
                                          self.data_dimensions.package_dim.boundnames()):
                                            data_line = data_line + (None,)
                                data_list.append(data_line)
                            return np.rec.array(data_list, self._recarray_type_list)
                        else:
                            return self.layer_storage[layer_check]._data_const_value is not None
                    else:
                        if return_data:
                            return self.layer_storage[0]._internal_data
                        else:
                            return True
            elif self.layer_storage[layer].data_storage_type == DataStorageType.internal_array:
                if return_data:
                ...
                if return_data:
                    ...
                    if data is None:
                        if self.layer_storage[layer].data_storage_type == DataStructureType.internal_constant:
                            return self.layer_storage[layer]._data_const_value[0]
                    else:
                        return data
                else:
                    return self.layer_storage[layer]._data_const_value is not None
            else:
                if return_data:
    ...
    def append_data(self, data):
        ...
        if self.layer_storage[0]._internal_data is None:
            ...
            if len(self.layer_storage[0]._internal_data[0]) < len(data[0]):
                ...
                for index in range(len(self.layer_storage[0]._internal_data[0]), len(data[0])):
                    self._duplicate_last_item()
                internal_data_list = self.layer_storage[0]._internal_data.tolist()
                for data_item in data:
                    internal_data_list.append(data_item)
                self._add_placeholders(internal_data_list)
                self.set_data(np.rec.array(internal_data_list, self._recarray_type_list))
            else:
                if len(self.layer_storage[0]._internal_data[0]) > len(data[0]):
                    # Add placeholders to data
                    self._add_placeholders(data)
                self.set_data(np.hstack((self.layer_storage[0]._internal_data,
                                         np.rec.array(data, self._recarray_type_list))))

    def set_data(self, data, layer=None, multiplier=[1.0], key=None, autofill=False):
        if self.data_structure_type == DataStructureType.recarray or \
          self.data_structure_type == DataStructureType.scalar:
            self._set_list(data, layer, multiplier, key, autofill)
        else:
            if self.data_dimensions.structure.name == 'aux':
                ...
                if len(data) == len(aux_var_names[0]) - 1:
                    for layer, aux_var_data in enumerate(data):
                        if layer > 0:
                            self.add_layer()
                        self._set_array(aux_var_data, layer, multiplier, key, autofill)
                else:
                    except_str = 'Unable to set data for aux variable.  Expected {} aux ' \
                                 'variables but got {}. {}'.format(len(aux_var_names[0]),
                                                                   len(data),
    ...
    def _set_list(self, data, layer, multiplier, key, autofill):
        if isinstance(data, dict):
            if 'filename' in data:
                ...
                return
        self.store_internal(data, layer, multiplier, key=key, autofill=autofill)
    ...
    def _set_array(self, data, layer, multiplier, key, autofill):
        ...
        if not self._set_array_layer(data, layer, multiplier, key):
            ...
            success = False
            if layer is None and self.data_structure_type == DataStructureType.ndarray and \
              len(data) == self.num_layers:
                self.layered = True
                # loop through list and try to store each list entry as a layer
                success = True
                for layer_num, layer_data in enumerate(data):
                    if not isinstance(layer_data, list) and not isinstance(layer_data, dict):
                        layer_data = [layer_data]
                    success = success and self._set_array_layer(layer_data, layer_num, multiplier, key)
            if not success:
                except_str = 'Unable to set data for variable "{}" layer {}.  Enable verbose mode for more ' \
                             'information. {}'.format(self.data_dimensions.structure.name,
                                                      layer,
                                                      self.data_dimensions.structure.path)
                print(except_str)
                raise MFDataException(except_str)
    ...
    def _set_array_layer(self, data, layer, multiplier, key):
        ...
        if isinstance(data, dict):
            if isinstance(data['data'], int) or isinstance(data['data'], float) or isinstance(data['data'], str):
            ...
            if 'filename' in data:
                ...
                if len(data['data']) == 1:
                    ...
                    if DatumUtil.is_float(multiplier):
                        ...
                        mult = 1
                    self.store_internal([data['data'][0] * multiplier], layer, True, [mult], key=key, print_format=iprn)
                else:
                    self.store_internal(data['data'], layer, False, [multiplier], key=key, print_format=iprn)
                return True
        elif isinstance(data[0], str):
            if data[0].lower() == 'internal':
                multiplier, iprn, flags_found = self.process_internal_line(data)
                self.store_internal(data[-1], layer, False, [multiplier], key=key, print_format=iprn)
                return True
            elif data[0].lower() != 'open/close':
                # assume open/close is just omitted, though test data file to be sure
                new_data = data[:]
    ...
    def get_active_layer_indices(self):
        ...
        for index in range(0, self.num_layers):
            if self.layer_storage[index].fname is not None or self.layer_storage[index]._internal_data is not None:
    ...
    def store_internal(self, data, layer=None, const=False, multiplier=[1.0], key=None, autofill=False,
                       print_format=None):
        if self.data_structure_type == DataStructureType.recarray:
            if self.layer_storage[0].data_storage_type == DataStorageType.internal_constant:
                self.layer_storage[0]._data_const_value = data
            else:
                self.layer_storage[0].data_storage_type = DataStorageType.internal_array
                if isinstance(data, np.recarray):
                    ...
                    if autofill and data is not None:
                        ...
                        data_item_structs = self.data_dimensions.structure.data_item_structures
                        if data_item_structs[0].tagged and not data_item_structs[0].type == 'keyword':
                            for data_index, data_entry in enumerate(data):
                                if (data_item_structs[0].type == 'string' and data_entry[0].lower() ==
                                  data_item_structs[0].name.lower()):
                                    break
                                data[data_index] = (data_item_structs[0].name.lower(),) + data[data_index]
                    self.build_type_list(data=data, key=key)
                    if autofill and data is not None:
                        # resolve any fields with data types that do not agree with the expected type list
                        self._resolve_multitype_fields(data)
                    if isinstance(data, list):
                        # data needs to be stored as tuples within a list.  if this is not the case try to fix it
                        self._tupleize_data(data)
                        # add placeholders to data so it agrees with expected dimensions of recarray
                        self._add_placeholders(data)
                    self.set_data(np.rec.array(data, self._recarray_type_list))
            ...
            if const:
                self.layer_storage[layer].data_storage_type = DataStorageType.internal_constant
                self.layer_storage[layer]._data_const_value = data
            else:
                self.layer_storage[layer].data_storage_type = DataStorageType.internal_array
                self.layer_storage[layer]._internal_data = np.reshape(data, dimensions)
    ...
    def _resolve_multitype_fields(self, data):
        ...
        for data_entry in data:
            for index, data_val in enumerate(data_entry):
                if index < itype_len and self._recarray_type_list[index][1] != object and \
                  type(data_val) != self._recarray_type_list[index][1] and (type(data_val) != int or
                  self._recarray_type_list[index][1] != float):
                    # for inconsistent types use generic object type
                    self._recarray_type_list[index] = (self._recarray_type_list[index][0], object)

    def store_external(self, file_path, layer=None, multiplier=[1.0],
                       print_format=None, data=None, do_not_verify=False, binary=False):
        layer, multiplier = self._store_prep(layer, multiplier)
        self.layer_storage[layer].fname = file_path
        self.layer_storage[layer].iprn = print_format
        self.layer_storage[layer].binary = binary
        self.layer_storage[layer].data_storage_type = DataStorageType.external_file
        if self.data_structure_type == DataStructureType.recarray:
            ...
            if data is not None:
                ...
                for data_item in MultiListIter(data, True):
                    ...
                    fd.write('{} '.format(self.to_string(data_item[0],
                                                         self.data_dimensions.structure.data_item_structures[0].type)))
                    current_size += 1
                if current_size != data_size:
                    except_str = 'Not enough data for "{}" provided for file {}.  Expected {} but only ' \
    ...
    def external_to_external(self, new_external_file, multiplier=None, layer=None):
        # currently only support files containing ndarrays
        assert(self.data_structure_type == DataStructureType.ndarray)
        assert((layer is None and self.num_layers == 1) or (layer is not None and layer < self.num_layers))
        # get data storage
        if layer is None:
            layer = 1
        ...
        # update
        self.store_external(new_external_file, layer, [self.layer_storage[layer].factor], self.layer_storage[layer].iprn,
    ...
    def internal_to_external(self, new_external_file, multiplier=None, layer=None, print_format=None):
        if layer is None:
            self.store_external(new_external_file, layer, multiplier, print_format,
                                self.layer_storage[0]._internal_data)
        else:
            self.store_external(new_external_file, layer, multiplier, print_format,
                                self.layer_storage[layer]._internal_data)

    def read_data_from_file(self, layer, fd=None, multiplier=None, print_format=None):
        ...
        for line in fd:
            ...
            for data in arr_line:
                if data != '':
                    if current_size == data_size:
                        print('WARNING: More data found than expected in file {} for '
                              'data "{}".'.format(fd.name, self.data_dimensions.structure.path))
                        break
            ...
            if current_size == data_size:
                break
        if current_size != data_size:
            except_str = 'Not enough data in file {} for data "{}".  Expected {} but only ' \
                         'found {}.'.format(fd.name, self.data_dimensions.structure.path, data_size, current_size)
            print(except_str)
            if close_file:
                fd.close()
            raise MFDataException(except_str)
    ...
    def to_string(self, val, type, is_cellid=False, possible_cellid=False, force_upper_case=False):
        ...
        if is_cellid or (possible_cellid and isinstance(val, tuple)):
            ...
            if (abs(val) > self._simulation_data.scientific_notation_upper_threshold or
               abs(val) < self._simulation_data.scientific_notation_lower_threshold) and \
               val != 0:
                format_str = '{:.%dE}' % self._simulation_data.float_precision
            else:
                format_str = '{:%d.%df}' % (self._simulation_data.float_characters,
    ...
    def process_internal_line(self, arr_line):
        ...
        if isinstance(arr_line, list):
            if len(arr_line) < 2:
                except_str = 'ERROR: Data array "{}" contains a INTERNAL that is not followed by a ' \
            ...
            while index < len(arr_line):
                if isinstance(arr_line[index], str):
                    if arr_line[index].lower() == 'factor' and index + 1 < len(arr_line):
                        multiplier = self.convert_data(arr_line[index+1], self._data_type)
                        internal_modifiers_found = True
                        index += 2
                    elif arr_line[index].lower() == 'iprn' and index + 1 < len(arr_line):
    ...
    def process_open_close_line(self, arr_line, layer, store=True):
        ...
        if isinstance(arr_line, list):
            ...
            if index < len(arr_line):
                self.layer_storage[layer].comments = MFComment(' '.join(arr_line[index:]), self.data_dimensions.structure.path,
    ...
    def _add_placeholders(self, data):
        ...
        for data_line in data:
            ...
            if data_line_len < len(self._recarray_type_list):
                for index in range(data_line_len, len(self._recarray_type_list)):
    ...
    def _build_full_data(self, apply_multiplier=False):
        ...
        if dimensions[0] < 0:
            return None
        full_data = np.full(dimensions, np.nan, self.data_dimensions.structure.get_datum_type(True))
        ...
        for layer in range(0, layers_to_process):
            if self.layer_storage[layer].factor is not None and apply_multiplier:
            ...
            if self.layer_storage[layer].data_storage_type == DataStorageType.internal_array:
                if len(self.layer_storage[layer]._internal_data) > layer and self.layer_storage[layer]._internal_data[layer] is None:
                    return None
                if self.num_layers == 1 or not self.layered:
                    full_data = self.layer_storage[layer]._internal_data * mult
                else:
                    full_data[layer] = self.layer_storage[layer]._internal_data * mult
            elif self.layer_storage[layer].data_storage_type == DataStorageType.internal_constant:
                if self.num_layers == 1 or not self.layered:
    ...
    def _verify_data(self, data_iter, layer):
        ...
        if self.data_dimensions.structure.type == 'recarray' or self.data_dimensions.structure.type == 'record':
            data_types = self.data_dimensions.structure.get_data_item_types()
            # check to see if data contains the correct types and is a possibly correct size
            record_loc = 0
            actual_data_size = 0
            rows_of_data = 0
            for data_item in data_iter:
                ...
                if record_loc == len(data_types) - 1:
                    ...
                    record_loc += 1
            return rows_of_data > 0 and (rows_of_data < data_dimensions[0] or data_dimensions[0] == -1)
        else:
            ...
            for index in array_index_iter:
                ...
                if current_col == dimensions[1] - 1:
                    if data_array is None:
                        data_array = np.rec.array(data_line, self._recarray_type_list)
                    else:
                        data_array = np.hstack((data_array, np.rec.array(data_line, self._recarray_type_list)))
                    current_col = 0
                    data_line = ()
    ...
    def resolve_data_size(self, index):
        # Resolves the size of a given data element based on the names in the existing rec_array
        # Assumes repeating data element names follow the format <data_element_name>_X
        assert(self.data_structure_type == DataStructureType.recarray)
        ...
        if len(label_list) == 1:
            return 1
        for forward_index in range(index+1, len(self.layer_storage[0]._internal_data.dtype.names)):
            forward_label = self.layer_storage[0]._internal_data.dtype.names[forward_index]
            forward_label_list = forward_label.split('_')
            if forward_label_list[0] != label_list[0]:
                return forward_index - index
        return len(self.layer_storage[0]._internal_data.dtype.names) - index

    def build_type_list(self, data_set=None, record_with_record=False, data=None, resolve_data_shape=True, key=None,
                        nseg=None):
        if data_set is None:
            self._recarray_type_list = []
            data_set = self.data_dimensions.structure
        initial_keyword = True
        for data_item, index in zip(data_set.data_item_structures, range(0, len(data_set.data_item_structures))):
            # handle optional mnames
            if not data_item.optional or len(data_item.name) < 5 or data_item.name.lower()[0:5] != 'mname' \
              or not self.in_model:
                ...
                if data_item.name.lower() == 'aux' and resolve_data_shape:
                    ...
                    if aux_var_names is not None:
                        for aux_var_name in aux_var_names[0]:
                            if aux_var_name.lower() != 'auxiliary':
                                self._recarray_type_list.append((aux_var_name, data_type))
                    ...
                    self._recarray_type_list.append((data_item.name, data_type))
                    ...
                    self._recarray_type_list.append((ks_data_item.name, ks_data_item.get_rec_type()))
                    if index == len(data_set.data_item_structures) - 1:
                        idx = 1
                        data_line_max_size = self._get_max_data_line_size(data)
                        while data is not None and len(self._recarray_type_list) < data_line_max_size:
                            # keystrings at the end of a line can contain items of variable length.
                            # assume everything at the end of the data line is related to the last keystring
                            self._recarray_type_list.append(('{}_{}'.format(ks_data_item.name, idx),
                                                             ks_data_item.get_rec_type()))
                            idx += 1

                elif data_item.name != 'boundname' or self.data_dimensions.package_dim.boundnames():
                    if data_item.type != 'keyword' or initial_keyword == False:  # don't include initial keywords
                        initial_keyword = False
                        shape_rule = None
                        if data_item.tagged:
                            if data_item.type != 'string' and data_item.type != 'keyword':
                                self._recarray_type_list.append(('{}_label'.format(data_item.name), object))
                        if nseg is not None and len(data_item.shape) > 0 and isinstance(data_item.shape[0], str) and \
                          data_item.shape[0][0:4] == 'nseg':
                            # nseg explicitly specified.  resolve any formula nseg is in
                            model_dim = self.data_dimensions.get_model_dim(None)
                            expression_array = model_dim.build_shape_expression(data_item.shape)
                            if isinstance(expression_array, list) and len(expression_array) == 1:
                                resolved_shape = [model_dim.resolve_exp(expression_array[0], nseg)]
                            else:
                                resolved_shape = [1]
                        else:
                            if resolve_data_shape:
                                resolved_shape, shape_rule = self.data_dimensions.get_data_shape(data_item, data_set, data, key)
                            else:
                                resolved_shape = [1]
                        if not resolved_shape or len(resolved_shape) == 0 or resolved_shape[0] == -1:
                        ...
                        if data_item.is_cellid:
                            if data_item.shape is not None and len(data_item.shape) > 0 and \
                              data_item.shape[0] == 'ncelldim':
                                # A cellid is a single entry (tuple) in the recarray.  Adjust dimensions accordingly.
                                cellid_size = self.data_dimensions.get_model_grid().get_num_spatial_coordinates()
                        ...
                        for index in range(0, resolved_shape[0]):
                            if resolved_shape[0] > 1:
                                ...
                                self._recarray_type_list.append(('{}_{}'.format(data_item.name, index), data_type))
                            else:
                                self._recarray_type_list.append((data_item.name, data_type))
    ...
    def _store_prep(self, layer, multiplier):
        assert(layer is None or layer < self.num_layers)
        if layer is None:
            # layer is none means the data provided is for all layers or this is not layered data
            layer = 0
            self.num_layers = 1
            self._internal_data = [None]
...
class MFTransient(object):
    """
    Parent class for transient data.  This class contains internal objects and methods that most end
    users will not need to access directly.
    ...
    get_data_prep(transient_key)
        called prior to the child class getting data.  ensures that the data retrieved will
        come from the dataset of a specific transient_key
    _set_data_prep(transient_key)
        called prior to the child class setting data.  ensures that the data set will go to the
        dataset of a specific transient_key
    _get_file_entry_prep(transient_key)
        called prior to the child class getting the file entry.  ensures that the file entry only reflects
        the data from a specific transient_key
    _load_prep(first_line, file_handle, block_header, pre_data_comments)
        called prior to the child class loading data from a file.  figures out what transient_key to
        store the data under
    _append_list_as_record_prep(record, transient_key)
        called prior to the child class appending a list to a record.  ensures that the list gets appended
        to the record associated with the key transient_key
    _update_record_prep(transient_key)
        called prior to the child class updating a record.  ensures that the record being updated is the
        one associated with the key transient_key
    get_active_key_list() : list
        returns a list of the active transient keys
    _verify_sp(sp_num) : bool
        returns true of the stress period sp_num is within the expected range of stress periods for this
        model
    ...
    def _load_prep(self, first_line, file_handle, block_header, pre_data_comments=None):
...
class MFData(object):
    """
    Base class for all data.  This class contains internal objects and methods that most end
    users will not need to access directly.
    ...
    path : tuple
        tuple describing path to the data generally in the format (<model>, <package>, <block>, <data>)
    dimensions : DataDimensions
        object used to retrieve dimension information about data
    *arges, **kwargs : exists to support different child class parameter sets with extra init parameters
    ...
    """
    def __init__(self, sim_data, structure, enable=True, path=None, dimensions=None, *args, **kwargs):
        # initialize
        self._current_key = None
        self._simulation_data = sim_data
        ...
        if self._simulation_data is not None:
            ...
            while self._path in self._simulation_data.mfdata:
                self._path = self._org_path[:-1] + ('{}_{}'.format(self._org_path[-1], index),)
                index += 1
    ...
    def aux_var_names(self):
        ...
        result = self._simulation_data.mfdata.find_in_path(parent_path, 'auxnames')
        ...
        for index, data_item in data_set.data_items.items():
            if data_item.type == 'record':
                ...
                if data_item.description:
                    if description:
                        description = '{}\n{}'.format(description, data_item.description)
                    else:
    ...
    def _structure_init(self, data_set=None):
        ...
        for data_item_struct in data_set.data_item_structures:
            if data_item_struct.type == 'record':
                ...
                if len(self.structure.data_item_structures) == 1: # and len(data_item_struct.shape) == 0:
                    # data item name is a keyword to look for
                    self._keyword = data_item_struct.name
    #            self.data_items.append(MFDataItem(data_item_struct))
...
#        assert(len(self.data_items) > 0)
    ...
    def _get_constant_formatting_string(self, const_val, layer, data_type, suffix='\n'):
        const_format = list(self._simulation_data.constant_formatting)
        const_format[1] = self._get_storage_obj().to_string(const_val, data_type)
        return '{}{}'.format(self._simulation_data.indent_string.join(const_format), suffix)
    ...
    def _get_aux_var_index(self, aux_name):
        ...
        if aux_var_names:
            for aux_var_name, index in zip(aux_var_names[0], range(0,len(aux_var_names[0]))):
    ...
    def _load_keyword(self, arr_line, index_num):
        ...
        if self._keyword != '':
            ...
            if not keyword_match and aux_var_index is None:
                aux_text = ''
                if aux_var_names is not None:
                    aux_text = ' or auxiliary variables {}'.format(aux_var_names[0])
                except_str = 'Error reading variable "{}".  Expected variable keyword "{}"{} not found ' \
    ...
    def _read_pre_data_comments(self, line, file_handle, pre_data_comments):
        ...
        if pre_data_comments:
            storage.pre_data_comments = MFComment(pre_data_comments.text, self._path, self._simulation_data, line_num)
        else:
        ...
        while MFComment.is_comment(arr_line, True) and line != '':
            if storage.pre_data_comments:
                ...
                storage.pre_data_comments = MFComment(arr_line, self._path, self._simulation_data, line_num)
    ...
    def _add_data_line_comment(self, comment, line_num):
        ...
        if line_num in storage.comments:
            ...
            storage.comments[line_num] = MFComment(' '.join(comment), self._path, self._simulation_data, line_num)
...
class MFMultiDimVar(MFData):
    def __init__(self, sim_data, structure, enable=True, path=None, dimensions=None):
        super(MFMultiDimVar, self).__init__(sim_data, structure, enable, path, dimensions)
    ...
    def _get_internal_formatting_string(self, layer):
        ...
        if self._get_storage_obj().layer_storage[layer].factor is not None:
            int_format.append(str(self._get_storage_obj().layer_storage[layer].factor))
        else:
            if data_type == 'float' or data_type == 'double' or data_type == 'double precision':
                int_format.append('1.0')
            else:
                int_format.append('1')
        if self._get_storage_obj().layer_storage[layer].iprn is not None:
            int_format.append('IPRN')
            int_format.append(str(self._get_storage_obj().layer_storage[layer].iprn))
    ...
    def _get_external_formatting_string(self, layer, ext_file_action):
        ...
        ext_file_path = file_mgmt.get_updated_path(self._get_storage_obj().layer_storage[layer].fname,
                                                   self._data_dimensions.package_dim.model_dim[0].model_name,
                                                   ext_file_action)
        ext_format = ['OPEN/CLOSE', "'{}'".format(ext_file_path)]
        ext_format.append('FACTOR')
        if self._get_storage_obj().layer_storage[layer].factor is not None:
            ext_format.append(str(self._get_storage_obj().layer_storage[layer].factor))
        else:
            if self.structure.type.lower() == 'float' or self.structure.type.lower() == 'double':
                ext_format.append('1.0')
            else:
                ext_format.append('1')
        if self._get_storage_obj().layer_storage[layer].binary == True:
            ext_format.append('BINARY')
        if self._get_storage_obj().layer_storage[layer].iprn is not None:
            ext_format.append('IPRN')
            ext_format.append(str(self._get_storage_obj().layer_storage[layer].iprn))
