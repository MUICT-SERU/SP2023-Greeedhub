...
class ModflowGwfwel(mfpackage.MFPackage):
    ...
    auto_flow_reduce : double
        * auto_flow_reduce (double) keyword and real value that defines the
        ...
        adjusted to 0 or a smaller negative value when the head in the cell is
        equal to or less than the calculated interval above the cell bottom.
        auto\_flow\_reduce is set to 0.1 if the specified value is
        less than or equal to zero. By default, negative pumping rates are not
    ...
    obs_filerecord : [obs6_filename]
        * obs6_filename (string) name of input file to define observations for
        ...
        obstype lists observation type(s) supported by the Well
        package.
    mover : boolean
    ...
    periodrecarray : [cellid, q, aux, boundname]
        ...
        * q (double) is the volumetric well rate. A positive value indicates
        ...
        (extraction). If the Options block includes a TIMESERIESFILE entry (see
