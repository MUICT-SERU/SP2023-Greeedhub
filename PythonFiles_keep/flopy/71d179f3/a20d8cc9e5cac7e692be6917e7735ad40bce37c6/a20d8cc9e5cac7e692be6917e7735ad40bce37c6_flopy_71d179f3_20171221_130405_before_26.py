...
class ModflowGwfwel(mfpackage.MFPackage):
    ...
    boundnames : (boundnames : boolean)
        boundnames : keyword to indicate that boundary names may be provided
          with the list of well cells.
    print_input : (print_input : boolean)
        print_input : keyword to indicate that the list of well information
          will be written to the listing file immediately after it is read.
    print_flows : (print_flows : boolean)
        print_flows : keyword to indicate that the list of well flow rates will
    ...
    save_flows : (save_flows : boolean)
        save_flows : keyword to indicate that well flow terms will be written
          to the file specified with ``BUDGET FILEOUT'' in Output Control.
    auto_flow_reduce : (auto_flow_reduce : double)
        auto_flow_reduce : keyword and real value that defines the fraction of
          the cell thickness used as an interval for smoothly adjusting
          negative pumping rates to 0 in cells with head values less than or
          equal to the bottom of the cell. Negative pumping rates are adjusted
          to 0 or a smaller negative value when the head in the cell is equal
          to or less than the calculated interval above the cell bottom.
          auto\_flow\_reduce is set to 0.1 if the specified value is
          less than or equal to zero. By default, negative pumping rates are
    ...
    obs_filerecord : [(obs6_filename : string)]
        obs6_filename : name of input file to define observations for the Well
    ...
    mover : (mover : boolean)
        mover : keyword to indicate that this instance of the Well Package can
    ...
    maxbound : (maxbound : integer)
        maxbound : integer value specifying the maximum number of wells cells
          that will be specified for use during any stress period.
    periodrecarray : [(cellid : (integer, ...)), (q : double), (aux : double),
      (boundname : string)]
        ...
        q : is the volumetric well rate. A positive value indicates recharge
          (injection) and a negative value indicates discharge (extraction). If
        ...
        aux : represents the values of the auxiliary variables for each well.
          The values of auxiliary variables must be present for each well. The
        ...
        boundname : name of the well cell. boundname is an ASCII character
    ...
    obs_filerecord = ListTemplateGenerator(('gwf6', 'wel', 'options', 
                                            'obs_filerecord'))
    periodrecarray = ListTemplateGenerator(('gwf6', 'wel', 'period', 
                                            'periodrecarray'))
    package_abbr = "gwfwel"
    package_type = "wel"
    ...
    def __init__(self, model, add_to_package_list=True, auxiliary=None,
                 ...
                 maxbound=None, periodrecarray=None, fname=None, pname=None,
                 parent_file=None):
