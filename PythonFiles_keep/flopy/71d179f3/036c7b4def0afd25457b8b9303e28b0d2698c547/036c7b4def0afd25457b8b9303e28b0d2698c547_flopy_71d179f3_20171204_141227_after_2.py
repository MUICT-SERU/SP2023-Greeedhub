...
class MFArray(mfdata.MFMultiDimVar):
    ...
    store_as_external_file : (external_file_path : string, multiplier : float, layer_num : int)
        Stores data from layer "layer_num" to an external file at "external_file_path" with a multiplier
        "multiplier".  For unlayered data do not pass in "layer".
    store_as_internal_array : (multiplier : float, layer_num : int)
        Stores data from layer "layer_num" internally within the MODFLOW file with a multiplier "multiplier".
    ...
    set_data : (data : ndarray/list, multiplier : float, layer_num : int)
        ...
        4) {'filename':filename, 'factor':fct, 'iprn':print} - dictionary defining external file information
        5) {'data':data, 'factor':fct, 'iprn':print) - dictionary defining internal information
        Data that is layered can also be set by defining a list with a length equal to the number of layers in the
        model.  Each layer in the list contains the data as defined in the formats above:
        ...
        if storage.layered and isinstance(k, int):
            ...
            if isinstance(k, int):
                if len(self.get_data(apply_mult=True).shape) == 1:
                    ...
                    raise Exception(
                        "mfdataarray.__getitem__() error: an integer was passed, " +
    ...
    def __setitem__(self, k, value):
        ...
        if storage.layered:
            ...
            self._get_storage_obj().set_data(a, key=self._current_key, multiplier=storage.layer_storage[0].factor)
    ...
    def supports_layered(self):
        model_grid = self._data_dimensions.get_model_grid()
        return self.structure.layered and model_grid.grid_type() != DiscretizationType.DISU

    def set_layered_data(self, layered_data):
        if layered_data is True and self.structure.layered is False:
            if self._data_dimensions.get_model_grid().grid_type() == DiscretizationType.DISU:
    ...
    def store_as_external_file(self, external_file_path, multiplier=[1.0], layer_num=None):
        if self._get_storage_obj() is None:
            self._data_storage = self._new_storage(False)
        ds_index = self._resolve_layer_index(layer_num)

        # move data to file
        if self._get_storage_obj().layer_storage[ds_index[0]].data_storage_type == mfdata.DataStorageType.external_file:
            self._get_storage_obj().external_to_external(external_file_path, multiplier, layer_num)
        else:
            self._get_storage_obj().internal_to_external(external_file_path, multiplier, layer_num)

        # update data storage
        self._get_storage_obj().layer_storage[ds_index[0]].data_storage_type = mfdata.DataStorageType.external_file
        self._get_storage_obj().layer_storage[ds_index[0]].fname = external_file_path
        if multiplier is not None:
            self._get_storage_obj().layer_storage[ds_index[0]].multiplier = multiplier[0]
    ...
    def set_data(self, data, multiplier=[1.0], layer_num=None):
        if self._get_storage_obj() is None:
            self._data_storage = self._new_storage(False)
        self._get_storage_obj().set_data(data, layer_num, multiplier, key=self._current_key)
    ...
    def load(self, first_line, file_handle, block_header, pre_data_comments=None):
        ...
        if len(arr_line) > 2:
            ...
            if arr_line[1].upper() == 'TIMEARRAYSERIES':
                tas_names = self._data_dimensions.package_dim.get_tasnames()
                if arr_line[2].lower() in tas_names:
                    # this is a time series array with a valid tas variable
                    storage.data_structure_type = mfdata.DataStructureType.scalar
                    storage.set_data(' '.join(arr_line[1:3]), 0, key=self._current_key)
                    return [False, None]
                else:
                    except_str = 'ERROR: "timearrayseries" keyword not followed by a valid TAS variable. ' \
                                 '{}'.format(self._path)
                    print(except_str)
        ...
        if self.structure.layered or aux_var_index is not None:
            if (len(arr_line) > index_num and arr_line[index_num].lower() == 'layered'):
                ...
                layers = len(self._data_dimensions.package_dim.get_aux_variables()[0]) - 1
    ...
    def _load_layer(self, layer, layer_size, storage, arr_line, file_handle):
        if not self.structure.data_item_structures[0].just_data or layer > 0:
            arr_line = mfdatautil.ArrayUtil.split_data_line(file_handle.readline())
        # if constant
        if arr_line[0].upper() == 'CONSTANT':
            if len(arr_line) < 2:
                except_str = 'ERROR: MFArray "{}" contains a CONSTANT that is not ' \
                             'followed by a number. {}'.format((self._data_name, self._path))
                print(except_str)
                raise mfstructure.MFFileParseException(except_str)
            # store data
            storage.layer_storage[layer].data_storage_type = mfdata.DataStorageType.internal_constant
            storage.store_internal([storage.convert_data(arr_line[1], self._data_type)],
                                   layer, const=True, multiplier=[1.0])
            # store anything else as a comment
            if len(arr_line) > 2:
                storage.layer_storage[layer].comments = mfdata.MFComment(' '.join(arr_line[2:]), self._path,
        ...
        # if internal
            ...
            if len(arr_line) < 2:
                except_str = 'ERROR: Data array "{}" contains a INTERNAL that is not followed by a ' \
                             'multiplier. {}'.format(self.structure.name, self._path)
                print(except_str)
                raise mfstructure.MFFileParseException(except_str)
            multiplier, print_format, flags_found = storage.process_internal_line(arr_line)

            storage.layer_storage[layer].data_storage_type = mfdata.DataStorageType.internal_array

            # store anything else as a comment
            if len(arr_line) > 5:
                storage.layer_storage[layer].comments = mfdata.MFComment(' '.join(arr_line[5:]), self._path,
                                                                         self._simulation_data, layer)

            # load variable data from current file
            data_from_file = storage.read_data_from_file(layer, file_handle, multiplier, print_format)
            data_shaped = self._resolve_data_shape(data_from_file[0])
            storage.store_internal(data_shaped, layer, const=False, multiplier=[multiplier], print_format=print_format)

            # verify correct size
            if layer_size > 0 and layer_size != data_from_file[1]:
                except_str = 'ERROR: Data array "{}" does not contain the expected amount of INTERNAL data.  ' \
                             'expected {}, found {}.  {}'.format(self.structure.name,
                                                                 layer_size,
                                                                 data_from_file[1], self._path)
                print(except_str)
                raise mfstructure.MFFileParseException(except_str)
#            elif arr_line[0].upper() == 'EXTERNAL' or arr_line[0].upper() == 'OPEN/CLOSE':
        elif arr_line[0].upper() == 'OPEN/CLOSE':
            storage.process_open_close_line(arr_line, layer)

    def get_file_entry(self, layer=None, ext_file_action=ExtFileAction.copy_relative_paths):
        data_storage = self._get_storage_obj()
        if data_storage is None or data_storage.num_layers == 0 or not data_storage.has_data():
            return ''
        ...
        if self._number_of_layers == 1:
            ...
            data_indent = '{}{}'.format(indent, self._simulation_data.indent_string)

        file_entry_array = []
        if data_storage.data_structure_type == mfdata.DataStructureType.scalar:
            # scalar data, like in the case of a time array series gets written on a single line
            file_entry_array.append('{}{}{}{}\n'.format(indent, self.structure.name, indent, data_storage.get_data()))
        elif data_storage.layered:
            if not layered_aux:
                if not self.structure.data_item_structures[0].just_data:
                    file_entry_array.append('{}{}{}{}\n'.format(indent, self.structure.name, indent, 'LAYERED'))
                else:
            ...
            if layer is None:
                ...
                if layer >= self._number_of_layers - 1:
                    except_str = 'Layer {} for variable "{}" does not exist. {}'.format(layer,
                                                                                        self._data_name,
                                                                                        self._path)
                    print(except_str)
                    raise mfstructure.MFDataException(except_str)
            ...
            for layer in range(layer_min, layer_max):
                file_entry_array.append(self._get_file_entry_layer(layer, data_indent,
                                                                   data_storage.layer_storage[layer].data_storage_type,
                                                                   ext_file_action, layered_aux))
        else:
            # data is not layered
            if not self.structure.data_item_structures[0].just_data:
                file_entry_array.append('{}{}\n'.format(indent, self.structure.name))
            file_entry_array.append(self._get_file_entry_layer(None, data_indent,
                                                               data_storage.layer_storage[0].data_storage_type,
                                                               ext_file_action))
    ...
    def _new_storage(self, set_layers=True):
        if set_layers:
            return mfdata.DataStorage(self._simulation_data, self._data_dimensions,
    ...
    def _get_file_entry_layer(self, layer, data_indent, storage_type, ext_file_action, layered_aux=False):
        if not self.structure.data_item_structures[0].just_data and not layered_aux:
            indent_string = '{}{}'.format(self._simulation_data.indent_string, self._simulation_data.indent_string)
        else:
        ...
        if layered_aux:
            ...
            indent_string = '{}{}'.format(indent_string, self._simulation_data.indent_string)

        data_storage = self._get_storage_obj()
        if storage_type == mfdata.DataStorageType.internal_array:
            # internal data header + data
            file_entry = '{}{}{}\n{}{}'.format(file_entry, indent_string,
                                               self._get_internal_formatting_string(layer).upper(),
                                               indent_string,
                                               self._get_data_layer_string(layer, data_indent).upper())
        elif storage_type == mfdata.DataStorageType.internal_constant:
            #  constant data
            file_entry = '{}{}{}'.format(file_entry, indent_string,
                                         self._get_constant_formatting_string(data_storage.get_const_val(layer),
                                                                              layer, self._data_type).upper())
        else:
            #  external data
            file_entry = '{}{}{}'.format(file_entry, indent_string,
    ...
    def _get_data_layer_string(self, layer, data_indent):
        ...
        data_iter = mfdatautil.ArrayUtil.next_item(self._get_storage_obj().get_data(layer, False))
        for item, last_item, new_list, nesting_change in data_iter:
            # increment data/layer counts
            line_data_count += 1
            layer_data_string[-1] = '{}{}{}'.format(layer_data_string[-1], self._simulation_data.indent_string,
                                                self._get_storage_obj().to_string(item, self._data_type))
    ...
    def _resolve_data_shape(self, data):
        dimensions, shape_rule = self._data_dimensions.get_data_shape(repeating_key=self._current_key)
        ...
        if self._get_storage_obj().layered:
            if layer_num is None:
                if allow_multiple_layers:
                    layer_index = self._get_storage_obj().get_active_layer_indices()
                else:
                    except_str = 'Data "{}" is layered but no layer_num was specified. ' \
...
class MFTransientArray(MFArray, mfdata.MFTransient):
    ...
    def __init__(self, sim_data, structure, enable=True, path=None, dimensions=None):
        ...
        self._transient_setup(self._data_storage, mfdata.DataStructureType.ndarray)
        self.repeating = True

    def add_transient_key(self, transient_key):
        super(MFTransientArray, self).add_transient_key(transient_key)
        self._data_storage[transient_key] = super(MFTransientArray, self)._new_storage()
    ...
    def set_data(self, data, multiplier=[1.0], layer_num=None, key=None):
        if isinstance(data, dict) or isinstance(data, OrderedDict):
            ...
            for key, list_item in data.items():
                ...
                super(MFTransientArray, self).set_data(list_item, multiplier, layer_num)
        else:
            if key is None:
                # search for a key
                new_key_index = self.structure.first_non_keyword_index()
                if new_key_index is not None and hasattr(data, '__len__') and len(data) > new_key_index:
    ...
    def get_file_entry(self, key=0, ext_file_action=ExtFileAction.copy_relative_paths):
        ...
        return super(MFTransientArray, self).get_file_entry(ext_file_action=ext_file_action)

    def load(self, first_line, file_handle, block_header, pre_data_comments=None):
        self._load_prep(first_line, file_handle, block_header, pre_data_comments)
        return super(MFTransientArray, self).load(first_line, file_handle, pre_data_comments)
