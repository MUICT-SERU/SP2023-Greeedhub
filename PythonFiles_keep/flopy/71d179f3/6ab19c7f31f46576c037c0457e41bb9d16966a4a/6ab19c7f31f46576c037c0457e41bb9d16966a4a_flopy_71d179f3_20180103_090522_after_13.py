...
class ModflowGwfhfb(mfpackage.MFPackage):
    ...
    print_input : boolean
        * print_input (boolean) keyword to indicate that the list of horizontal
        flow barriers will be written to the listing file immediately after it
        is read.
    maxhfb : integer
        * maxhfb (integer) integer value specifying the maximum number of
        ...
        value of maxhfb is used to allocate memory for the horizontal
        flow barriers.
    hfbrecarray : [cellid1, cellid2, hydchr]
        * cellid1 ((integer, ...)) identifier for the first cell. For a
        structured grid that uses the DIS input file, cellid1 is the
        layer, row, and column numbers of the cell. For a grid that uses the
        DISV input file, cellid1 is the layer number and cell2d number
        for the two cells. If the model uses the unstructured discretization
        (DISU) input file, then cellid1 is the node numbers for the
        cell. The barrier is located between cells designated as
        cellid1 and cellid2. For models that use the DIS and
        DISV grid types, the layer number for cellid1 and
        cellid2 must be the same. For all grid types, cells must be
        horizontally adjacent or the program will terminate with an error.
        * cellid2 ((integer, ...)) identifier for the second cell. See
        cellid1 for description of how to specify.
        * hydchr (double) is the hydraulic characteristic of the horizontal-
        ...
        barrier. For example, if the value for hydchr was specified as
        1.5, the conductance calculated for the two cells would be multiplied
        by 1.5.
