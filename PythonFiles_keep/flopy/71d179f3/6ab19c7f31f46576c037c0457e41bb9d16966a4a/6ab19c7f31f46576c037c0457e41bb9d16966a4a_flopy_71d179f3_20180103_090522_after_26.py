...
class ModflowGwfuzf(mfpackage.MFPackage):
    ...
    obs_filerecord : [obs6_filename]
        * obs6_filename (string) name of input file to define observations for
        ...
        obstype lists observation type(s) supported by the UZF
        package.
    mover : boolean
    ...
    simulate_et : boolean
        * simulate_et (boolean) keyword specifying that ET in the unsaturated
        (UZF) and saturated zones (GWF) will be simulated. ET can be simulated
        in the UZF cell and not the GWF cell by emitting keywords
    ...
    square_gwet : boolean
        * square_gwet (boolean) keyword specifying that groundwater ET will be
        simulated by assuming a constant ET rate for groundwater levels between
        land surface (TOP) and land surface minus the ET extinction
        depth (TOP-EXTDP). Groundwater ET is smoothly reduced from the
    ...
    simulate_gwseep : boolean
        * simulate_gwseep (boolean) keyword specifying that groundwater
        discharge (GWSEEP) to land surface will be simulated.
        Groundwater discharge is nonzero when groundwater head is greater than
        land surface.
    unsat_etwc : boolean
        * unsat_etwc (boolean) keyword specifying that ET in the unsaturated
        ...
        the water content (THETA) is greater than the ET extinction
        water content (extwc).
    unsat_etae : boolean
    ...
    nuzfcells : integer
        * nuzfcells (integer) is the number of UZF cells. More than 1 UZF cell
        can be assigned to a GWF cell; however, only 1 GWF cell can be assigned
        to a single UZF cell. If the MULTILAYER option is used then UZF cells
        can be assigned to GWF cells below (in deeper layers than) the upper
        most active GWF cells.
    ntrailwaves : integer
    ...
    nwavesets : integer
        * nwavesets (integer) is the number of UZF cells specified. NWAVSETS
        ...
        required to resolve variations in water content within the unsaturated
        zone.
    uzfrecarray : [iuzno, cellid, landflag, ivertcon, surfdep, vks, thtr, thts,
      thti, eps, boundname]
        * iuzno (integer) integer value that defines the UZF cell number
        ...
        iuzno must be greater than zero and less than or equal to
        nuzfcells. UZF information must be specified for every UZF
        cell or the program will terminate with an error. The program will also
        terminate with an error if information for a UZF cell is specified more
        than once.
        ...
        * ivertcon (integer) integer value set to specify underlying UZF cell
        that receives water flowing to bottom of cell. If unsaturated zone flow
        reaches water table before the cell bottom then water is added to GWF
        cell instead of flowing to underlying UZF cell. A value of 0 indicates
        the UZF cell is not connected to an underlying UZF cell.
    ...
    uzfperiodrecarray : [iuzno, finf, pet, extdp, extwc, ha, hroot, rootact, aux]
        ...
        * finf (string) real or character value that defines the applied
        infiltration rate of the UZF cell ($LT^{-1$). If the Options block
        ...
        * pet (string) real or character value that defines the potential
        ...
        underlying UZF and GWF cells. pet is always specified, but is
        only used if SIMULATE\_ET is specified in the OPTIONS block.
        ...
        * extdp (string) real or character value that defines the
        ...
        ivertcon is greater than zero and extdp extends below
        the GWF cell bottom then remaining potential evapotranspiration is
        applied to the underlying UZF and GWF cells. extdp is always
        ...
        * extwc (string) real or character value that defines the
        ...
        extwc is always specified, but is only used if
        SIMULATE\_ET and UNSAT\_ETWC are specified in the
        OPTIONS block. If the Options block includes a TIMESERIESFILE
        ...
        * ha (string) real or character value that defines the air entry
        potential (head) of the UZF cell. ha is always specified, but
        is only used if SIMULATE\_ET and UNSAT\_ETAE are
        specified in the OPTIONS block. If the Options block includes a
        TIMESERIESFILE entry (see the ``Time-Variable Input''
        section), values can be obtained from a time series by entering the
        time-series name in place of a numeric value.
        * hroot (string) real or character value that defines the root
        potential (head) of the UZF cell. hroot is always specified,
        but is only used if SIMULATE\_ET and UNSAT\_ETAE are
        specified in the OPTIONS block. If the Options block includes a
        TIMESERIESFILE entry (see the ``Time-Variable Input''
        section), values can be obtained from a time series by entering the
        time-series name in place of a numeric value.
        * rootact (string) real or character value that defines the root
        activity function of the UZF cell. rootact is the length of
        roots in a given volume of soil divided by that volume. Values range
        from 0 to about 3 $cm^{-2$, depending on the plant community and its
        stage of development. rootact is always specified, but is only
        used if SIMULATE\_ET and UNSAT\_ETAE are specified in
        the OPTIONS block. If the Options block includes a
        TIMESERIESFILE entry (see the ``Time-Variable Input''
        section), values can be obtained from a time series by entering the
        time-series name in place of a numeric value.
