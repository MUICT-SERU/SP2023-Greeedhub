...
class ModflowGwfsfr(mfpackage.MFPackage):
    ...
    print_input : boolean
        * print_input (boolean) keyword to indicate that the list of stream
        reach information will be written to the listing file immediately after
        it is read.
    print_stage : boolean
        * print_stage (boolean) keyword to indicate that the list of stream
        ...
        is no Output Control option and PRINT\_STAGE is specified, then stages
    ...
    obs_filerecord : [obs6_filename]
        * obs6_filename (string) name of input file to define observations for
        ...
        obstype lists observation type(s) supported by the SFR
        package.
    mover : boolean
    ...
    maximum_depth_change : double
        * maximum_depth_change (double) value that defines the depth closure
        tolerance. By default, dmaxchg is equal to $1 \times 10^{-5$.
    unit_conversion : double
        * unit_conversion (double) value (or conversion factor) that is used in
        calculating stream depth for stream reach. A constant of 1.486 is used
        for flow units of cubic feet per second, and a constant of 1.0 is used
        for units of cubic meters per second. The constant must be multiplied
        by 86,400 when using time units of days in the simulation.
    nreaches : integer
        * nreaches (integer) integer value specifying the number of stream
        reaches. There must be nreaches entries in the PACKAGEDATA
        block.
    sfrrecarray : [rno, cellid, rlen, rwid, rgrd, rtp, rbth, rhk, man, ncon, ustrf,
      ndv, aux, boundname]
        * rno (integer) integer value that defines the reach number associated
        with the specified PACKAGEDATA data on the line. rno must be
        ...
        terminate with an error. The program will also terminate with an error
        if information for a reach is specified more than once.
        * cellid ((integer, ...)) The keyword `none' must be specified
        for reaches that are not connected to an underlying GWF cell. The
        keyword `none' is used for reaches that are in cells that have
        IDOMAIN values less than one or are in areas not covered by
        the GWF model grid. Reach-aquifer flow is not calculated if the keyword
        `none' is specified.
        * rlen (double) real value that defines the reach length. rlen
        must be greater than zero.
        * rwid (double) real value that defines the reach width. rwid
        must be greater than zero.
        ...
        * rbth (double) real value that defines the thickness of the reach
        streambed. rbth can be any value if cellid is
        `none'. Otherwise, rbth must be greater than zero.
        * rhk (double) real value that defines the hydraulic conductivity of
        the reach streambed. rhk can be any positive value if
        cellid is `none'. Otherwise, rhk must be
        greater than zero.
        * man (string) real or character value that defines the Manning's
        roughness coefficient for the reach. man must be greater than
        ...
        * ustrf (double) real value that defines the fraction of upstream flow
        ...
        reach. The sum of all ustrf values for all reaches connected
        to the same upstream reach must be equal to one and ustrf must
        ...
        * aux (double) represents the values of the auxiliary variables for
        ...
        for each stream reach. The values must be specified in the order of the
        ...
        * boundname (string) name of the stream reach cell. boundname is an
        ASCII character variable that can contain as many as 40 characters. If
        boundname contains spaces in it, then the entire name must be enclosed
        within single quotes.
    reach_connectivityrecarray : [rno, ic]
        * rno (integer) integer value that defines the reach number associated
        with the specified CONNECTIONDATA data on the line. rno must
        be greater than zero and less than or equal to NREACHES. Reach
        connection information must be specified for every reach or the program
        will terminate with an error. The program will also terminate with an
        error if connection information for a reach is specified more than
        once.
        * ic (integer) integer value that defines the reach number of the reach
        ...
        upstream or downstream end of the reach. Negative ic numbers
        indicate connected reaches are connected to the downstream end of the
        current reach. Positive ic numbers indicate connected reaches
    ...
    reach_diversionsrecarray : [rno, idv, iconr, cprior]
        * rno (integer) integer value that defines the reach number associated
        with the specified DIVERSIONS data on the line. rno must be
        greater than zero and less than or equal to NREACHES. Reach
        diversion information must be specified for every reach with a
        ndv value greater than 0 or the program will terminate with an
        error. The program will also terminate with an error if diversion
        ...
        * idv (integer) integer value that defines the downstream diversion
        number for the diversion for reach rno. idv must be
        greater than zero and less than or equal to ndv for reach
        rno.
        * iconr (integer) integer value that defines the downstream reach that
        will receive the diverted water. idv must be greater than zero
        and less than or equal to NREACHES. Furthermore, reach
        iconr must be a downstream connection for reach rno.
        * cprior (string) character string value that defines the the
        ...
        rno ($Q_{DS$); in this case, 0.0 $\le$ divflow $\le$
        1.0. (2) cprior = `EXCESS', a diversion is made only if
        $Q_{DS$ for reach rno exceeds the value of divflow.
        If this occurs, then the quantity of water diverted is the excess flow
        ($Q_{DS -$ divflow) and $Q_{DS$ from reach rno is
        set equal to divflow. This represents a flood-control type of
        diversion, as described by Danskin and Hanson (2002). (3)
        cprior = `THRESHOLD', then if $Q_{DS$ in reach rno
        is less than the specified diversion flow (divflow), no water
        is diverted from reach rno. If $Q_{DS$ in reach rno
        is greater than or equal to (divflow), (divflow) is
        diverted and $Q_{DS$ is set to the remainder ($Q_{DS -$
        divflow)). This approach assumes that once flow in the stream
        is sufficiently low, diversions from the stream cease, and is the
        `priority' algorithm that originally was programmed into the STR1
        Package (Prudic, 1989). (4) cprior = `UPTO' -- if $Q_{DS$ in
        reach rno is greater than or equal to the specified diversion
        flow (divflow), $Q_{DS$ is reduced by divflow. If
        $Q_{DS$ in reach rno is less than (divflow),
        divflow is set to $Q_{DS$ and there will be no flow available
        for reaches connected to downstream end of reach rno.
    reachperiodrecarray : [rno, sfrsetting]
        * rno (integer) integer value that defines the reach number associated
        with the specified PERIOD data on the line. rno must be
        greater than zero and less than or equal to NREACHES.
        * sfrsetting (keystring) line of information that is parsed into a
        ...
        sfrsetting string include: STATUS, MANNING,
        STAGE, INFLOW, RAINFALL,
    ...
    divrate : double
        * divrate (double) real or character value that defines the volumetric
        diversion (divflow) rate for the streamflow routing reach. If
        the Options block includes a TIMESERIESFILE entry (see the ``Time-
        Variable Input'' section), values can be obtained from a time series by
