...
if __name__ == "__main__":
    ...
    prox_A = po.prox_plus
    prox_S = po.prox_plus
    proxs_g = [None, [prox_ones]]
    Ls = [None, [np.ones((1,A0.shape[0]))]]
    
    ST, AT, tr = nmf.nmf(Y.T, S0, A0, prox_A=prox_A, Ls=Ls, proxs_g=proxs_g,
                       e_rel=1e-6, e_abs=1e-6/noise**2, traceback=True)
    S = ST.T
    A = AT.T
