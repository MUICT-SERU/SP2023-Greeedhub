...
class LinearPLUnitermExperiment(BasePLExperiment):
    ...
    def run_experiment(self, **kwargs):
        ...
        self.process_results(filtered_set.get_usls(), pipeline, query_word=words[0])
    ...
    def process_results(self, filtered_usl_pool, pipeline, **kwargs):
        ...
        usls_containing_query = [str(usl) for usl in filtered_usl_pool if kwargs["query_word"] in usl.texts[0].children]
...
if __name__ == "__main__":
    ...
    experiment= LinearPLUnitermExperiment()
    results = []
    for i in range(1):
