...
class ModflowGwfdrn(mfpackage.MFPackage):
    ...
    boundnames : (boundnames : boolean)
        boundnames : keyword to indicate that boundary names may be provided
          with the list of drain cells.
    print_input : (print_input : boolean)
        print_input : keyword to indicate that the list of drain information
          will be written to the listing file immediately after it is read.
    print_flows : (print_flows : boolean)
        print_flows : keyword to indicate that the list of drain flow rates
    ...
    save_flows : (save_flows : boolean)
        save_flows : keyword to indicate that drain flow terms will be written
    ...
    obs_filerecord : [(obs6_filename : string)]
        obs6_filename : name of input file to define observations for the Drain
    ...
    mover : (mover : boolean)
        mover : keyword to indicate that this instance of the Drain Package can
    ...
    maxbound : (maxbound : integer)
        maxbound : integer value specifying the maximum number of drains cells
          that will be specified for use during any stress period.
    periodrecarray : [(cellid : (integer, ...)), (elev : double), (cond : double),
      (aux : double), (boundname : string)]
        ...
        elev : is the elevation of the drain. If the Options block includes a
        ...
        cond : is the hydraulic conductance of the interface between the
          aquifer and the drain. If the Options block includes a TIMESERIESFILE
          ...
          of a numeric value.
        aux : represents the values of the auxiliary variables for each drain.
          The values of auxiliary variables must be present for each drain. The
        ...
        boundname : name of the drain cell. boundname is an ASCII character
