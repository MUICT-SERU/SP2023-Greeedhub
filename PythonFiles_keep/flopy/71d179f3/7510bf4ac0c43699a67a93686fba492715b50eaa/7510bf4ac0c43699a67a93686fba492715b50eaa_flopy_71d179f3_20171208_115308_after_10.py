...
class ModflowGwfghb(mfpackage.MFPackage):
    ...
    boundnames : (boundnames : boolean)
        boundnames : keyword to indicate that boundary names may be provided
          with the list of general-head boundary cells.
    print_input : (print_input : boolean)
        print_input : keyword to indicate that the list of general-head
          boundary information will be written to the listing file immediately
          after it is read.
    print_flows : (print_flows : boolean)
        print_flows : keyword to indicate that the list of general-head
    ...
    save_flows : (save_flows : boolean)
        save_flows : keyword to indicate that general-head boundary flow terms
    ...
    obs_filerecord : [(obs6_filename : string)]
        obs6_filename : name of input file to define observations for the
          General-Head Boundary package. See the ``Observation utility''
          section for instructions for preparing observation input files. Table
          obstype lists observation type(s) supported by the General-
          Head Boundary package.
    mover : (mover : boolean)
        mover : keyword to indicate that this instance of the General-Head
    ...
    maxbound : (maxbound : integer)
        maxbound : integer value specifying the maximum number of general-head
          boundary cells that will be specified for use during any stress
          period.
    periodrecarray : [(cellid : (integer, ...)), (bhead : double), (cond : double),
      (aux : double), (boundname : string)]
        ...
        bhead : is the boundary head. If the Options block includes a
        ...
        cond : is the hydraulic conductance of the interface between the
          aquifer cell and the boundary. If the Options block includes a
          TIMESERIESFILE entry (see the ``Time-Variable Input'' section),
          values can be obtained from a time series by entering the time-series
          name in place of a numeric value.
        aux : represents the values of the auxiliary variables for each
          general-head boundary. The values of auxiliary variables must be
        ...
        boundname : name of the general-head boundary cell. boundname is an
    ...
    obs_filerecord = ListTemplateGenerator(('gwf6', 'ghb', 'options', 
                                            'obs_filerecord'))
    periodrecarray = ListTemplateGenerator(('gwf6', 'ghb', 'period', 
                                            'periodrecarray'))
    package_abbr = "gwfghb"
