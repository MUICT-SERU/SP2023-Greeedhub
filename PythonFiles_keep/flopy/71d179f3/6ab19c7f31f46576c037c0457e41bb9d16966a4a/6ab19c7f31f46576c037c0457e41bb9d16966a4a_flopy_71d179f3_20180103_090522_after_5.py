...
class ModflowGwfdisu(mfpackage.MFPackage):
    ...
    xorigin : double
        * xorigin (double) x-position of the origin used for model grid
        ...
        system. A default value of zero is assigned if not specified. The value
        for xorigin does not affect the model simulation, but it is
    ...
    yorigin : double
        * yorigin (double) y-position of the origin used for model grid
        ...
        The value for yorigin does not affect the model simulation,
    ...
    angrot : double
        * angrot (double) counter-clockwise rotation angle (in degrees) of the
        ...
        system. If not specified, then a default value of 0.0 is assigned. The
        value for angrot does not affect the model simulation, but it
        is written to the binary grid file so that postprocessors can locate
        the grid in space.
    nodes : integer
        * nodes (integer) is the number of cells in the model grid.
    nja : integer
        * nja (integer) is the sum of the number of connections and
        nodes. When calculating the total number of connections, the
        connection between cell $n$ and cell $m$ is considered to be different
        from the connection between cell $m$ and cell $n$. Thus, nja
        is equal to the total number of connections, including $n$ to $m$ and
        $m$ to $n$, and the total number of cells.
    nvert : integer
        * nvert (integer) is the total number of (x, y) vertex pairs used to
        ...
        nvert is not specified or is specified as zero, then the
    ...
    iac : [integer]
        * iac (integer) is the number of connections (plus 1) for each cell.
        The sum of iac must be equal to nja.
    ja : [integer]
        * ja (integer) is a list of cell number (n) followed by its connecting
        ...
        cells and their connections to the other GWF cells. Also note that the
        JA list input may be chopped up to have every node number and its
    ...
    ihc : [integer]
        * ihc (integer) is an index array indicating the direction between node
        n and all of its m connections. If $ihc=0$ -- cell $n$ and cell $m$ are
        connected in the vertical direction. Cell $n$ overlies cell $m$ if the
        cell number for $n$ is less than $m$; cell $m$ overlies cell $n$ if the
        cell number for $m$ is less than $n$. If $ihc=1$ -- cell $n$ and cell
        $m$ are connected in the horizontal direction. If $ihc=2$ -- cell $n$
        and cell $m$ are connected in the horizontal direction, and the
        connection is vertically staggered. A vertically staggered connection
        is one in which a cell is horizontally connected to more than one cell
        in a horizontal connection.
    cl12 : [double]
    ...
    hwva : [double]
        * hwva (double) is a symmetric array of size nja. For
        horizontal connections, entries in hwva are the horizontal
        width perpendicular to flow. For vertical connections, entries in
        hwva are the vertical area for flow. Thus, values in the
        hwva array contain dimensions of both length and area. Entries
        in the hwva array have a one-to-one correspondence with the
        connections specified in the ja array. Likewise, there is a
        one-to-one correspondence between entries in the hwva array
        and entries in the ihc array, which specifies the connection
        type (horizontal or vertical). Entries in the hwva array must
        be symmetric; the program will terminate with an error if the value for
        hwva for an $n-m$ connection does not equal the value for
        hwva for the corresponding $m-n$ connection.
    angldegx : [double]
        * angldegx (double) is the angle (in degrees) between the horizontal
        ...
        angle varies between zero and 360.0 degrees. angldegx is only
        needed if horizontal anisotropy is specified in the NPF Package or if
        the XT3D option is used in the NPF Package. angldegx does not
        need to be specified if horizontal anisotropy or the XT3D option is not
        used. angldegx is of size nja; values specified for vertical
        connections and for the diagonal position are not used. Note that
        angldegx is read in degrees, which is different from MODFLOW-
        USG, which reads a similar variable (anglex) in radians.
    ...
    cell2drecarray : [icell2d, xc, yc, ncvert, icvert]
        ...
        * icvert (integer) is an array of integer values containing vertex
        numbers (in the VERTICES block) used to define the cell. Vertices must
