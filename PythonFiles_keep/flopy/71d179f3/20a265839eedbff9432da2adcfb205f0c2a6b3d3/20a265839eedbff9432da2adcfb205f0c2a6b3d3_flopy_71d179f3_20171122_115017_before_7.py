...
class ModflowGwfdrn(mfpackage.MFPackage):
    ...
    obs_filerecord : [(obs6 : keyword), (filein : keyword), (obs6_filename : string)]
        ...
        obs6_filename : name of input file to define observations for the Drain package. See the ``Observation utility'' section for instructions for preparing observation input files. Table obstype lists observation type(s) supported by the Drain package.
    mover : (mover : keyword)
    ...
    periodrecarray : [(cellid : integer), (elev : double), (cond : double), (aux : double), (boundname : string)]
    ...
    def __init__(self, model, add_to_package_list=True, auxiliary=None, auxmultname=None, boundnames=None,
        ...
        super(ModflowGwfdrn, self).__init__(model, "drn", fname, pname, add_to_package_list, parent_file)        
